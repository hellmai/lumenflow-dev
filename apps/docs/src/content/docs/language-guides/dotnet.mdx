...
<Aside>
  ...
  issues that linters catch in dynamic languages. For additional static analysis, configure Roslyn
  analyzers.
</Aside>
...
<Tabs>
  <TabItem label="Standard">
    ```yaml gates: execution: preset: 'dotnet' format: 'dotnet format --verify-no-changes --severity
  ...
  <TabItem label="With Stryker">
    ```yaml gates: execution: preset: 'dotnet' test: 'dotnet test && dotnet stryker --threshold-high
    80' ```
  </TabItem>
</Tabs>
