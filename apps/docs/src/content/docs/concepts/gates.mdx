---
title: Gates
description: Config-driven quality gates enforced as kernel policies
---
...
<Aside type="note" title="Gates are kernel-enforced policies">
  In the [Software Delivery Pack](/concepts/packs), gates are implemented as pack-layer policies
  with `trigger: on_completion`. The [policy engine](/concepts/policy-engine) evaluates them when a
  task tries to complete — an agent cannot skip gates by calling `completeTask` directly.
</Aside>
...
When `wu:prep` or `wu:done` runs, the [kernel](/concepts/kernel) evaluates these policies. A `deny` from any gate makes the completion decision final — the [deny-wins invariant](/concepts/policy-engine#deny-wins-evaluation) applies. The result is recorded in the [evidence store](/concepts/evidence) for audit.
...
- [Policy Engine](/concepts/policy-engine) — How gates are evaluated as deny-wins policies
- [Evidence Store](/concepts/evidence) — How gate results are recorded for audit
