...
<Aside type="note" title="Architecture layers">
  ...
  Lifecycle, Worktree Discipline, Gates Pipeline, Lane WIP, Memory) show concepts from the
  [Software Delivery Pack](/pack/overview) that build on top of the kernel.
</Aside>
...
call -> lookup -> scope -> reserved -> persist -> policy -> validate -> dispatch -> evidence
...
reserved -> denied: .lumenflow/** {
  style.stroke: "#d32f2f"
  style.stroke-dash: 3
}
policy -> denied: deny-wins {
  style.stroke: "#d32f2f"
  style.stroke-dash: 3
}
```
