...
<Aside type="note" title="Architecture layers">
  ...
  diagram shows the domain-agnostic [kernel](/concepts/kernel). The remaining diagrams (WU
  Lifecycle, Worktree Discipline, Gates Pipeline, Lane WIP, Memory) show concepts from the [Software
  Delivery Pack](/pack/overview) that build on top of the kernel.
</Aside>
...
Every tool call — from CLI, MCP, or programmatic API — passes through the [kernel](/concepts/kernel) before anything executes.
...
- Steps 2–5 are the authorization gate — denial is still recorded as [evidence](/concepts/evidence)
- [Scope intersection](/concepts/scope-intersection) computes workspace ∩ lane ∩ task ∩ tool permissions
- [Policy evaluation](/concepts/policy-engine) uses deny-wins cascade across 4 layers
- [Packs](/concepts/packs) provide the tool handlers; the kernel is domain-agnostic

---

## Tool Execution Stack

Inside step 7 (dispatch), tools execute through a three-layer stack. See [Tool Execution](/concepts/tool-execution) for the full architecture.
...
LumenFlow is a monorepo with clear dependency boundaries. See [Package Architecture](/concepts/package-architecture) for details.
...
Quality gates are [pack policies](/concepts/policy-engine#gates-as-policies) with `trigger: on_completion`, evaluated by the [kernel](/concepts/kernel) before a task can complete.
...
- [Tool Execution](/concepts/tool-execution) — How pack tools execute inside dispatch
- [Package Architecture](/concepts/package-architecture) — Package relationships and build flow
- [Packs](/concepts/packs) — How domain tools are loaded and dispatched
- [Work Units](/concepts/work-units) — Deep dive into WU structure
- [Lanes](/concepts/lanes) — Lane configuration and scope boundaries
