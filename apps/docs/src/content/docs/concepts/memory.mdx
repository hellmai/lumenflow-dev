...
Creates `.lumenflow/memory/` directory for storing checkpoints.
...
Start a new memory session, optionally linked to a WU.
...
pnpm mem:signal "Tests written for auth flow, starting integration tests"

# Document the key decision made
pnpm mem:decide \
  --question "How to handle token refresh?" \
  --answer "Silent refresh with retry queue" \
  --reason "Better UX, handles race conditions"
```
...
pnpm mem:decide \
  ...
  --answer "OpenTelemetry + Grafana" \
  --reason "Vendor neutral, good ecosystem"

pnpm mem:signal "Evaluated 3 platforms, OTel winning"
pnpm mem:checkpoint
...
<Aside type="note">
  Configure `progress_signals.enabled: true` in `.lumenflow.config.yaml` to make signaling
  mandatory for sub-agents. See [Progress Signals](#progress-signals) for all options.
</Aside>

```bash
# Agent A working on Core (WU-100)
pnpm mem:signal "New validation port added: ValidationService"
...
pnpm mem:signal "Auth flow 80% complete. Remaining: integration tests for refresh token. Files ready: src/auth/*.ts"

# 3. Document any in-progress decisions
pnpm mem:decide \
  --question "Error handling strategy?" \
  --answer "Pending - need to decide between throw vs Result type"
