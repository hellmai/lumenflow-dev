...
async function runExclusive<T>(operation: () => Promise<T>): Promise<T> {
  ...
  executionQueue = scheduled.then(() => undefined, () => undefined);
  return scheduled;
}
```
...
- [Kernel Runtime](/concepts/kernel) — The 8-step pipeline that dispatches tools
- [Package Architecture](/concepts/package-architecture) — How packages relate and how packs are distributed
