...
`workspace.yaml` (`software_delivery.lanes.definitions`) is authoritative for lane validation.
`.lumenflow.lane-inference.yaml` is derived lane taxonomy metadata used by suggestion/classification
commands and can be regenerated with:
