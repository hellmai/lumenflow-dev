...
| `lane_health`               | `"warn"` \| `"error"` \| `"off"` | `"warn"`          | Lane overlap enforcement mode (`warn` logs, `error` blocks, `off` disables).  |
| `co_change`                 | object[]                         | `[]`              | Co-change rules: trigger/require glob pairs with severity (`warn/error/off`). |
