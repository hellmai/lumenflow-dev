...
The workspace spec (`workspace.yaml`) is the kernel-level configuration file that defines the runtime environment for a LumenFlow workspace. It declares which [packs](/kernel/packs) are loaded, which [lanes](/packs/software-delivery/concepts/lanes) exist, what security boundaries apply, and where events and memory are stored.
...
When a pack declares a `config_key` in its [manifest](/packs/software-delivery/setup/create-a-pack#pack-configuration), that key becomes a valid root-level field in `workspace.yaml`. For example, the Software Delivery pack declares `config_key: software_delivery`, which makes the `software_delivery` root key available.
...
<Aside type="tip">
  ...
  declare `config_key: customer_support`, making `customer_support.*` keys writable via
  `config:set`. See the [Create a Pack](/packs/software-delivery/setup/create-a-pack) guide for
  details.
</Aside>

## Pack Pins

Each entry in the `packs` array is a **pack pin** — a versioned, integrity-checked reference to a domain pack that the [kernel](/kernel/runtime) loads at startup.
...
The lane's `allowed_scopes` are used as the second level of the [scope intersection](/kernel/scope-intersection) algorithm:
...
Network scopes participate in [scope intersection](/kernel/scope-intersection) across all four layers (workspace, lane, task, tool). See the scope intersection page for the deny-wins semantics and entry-level set intersection behavior.
...
<Aside type="caution">
  The `software_delivery` block is validated in two phases. Phase 1 validates kernel root keys at
  startup. Phase 2 resolves pack manifest `config_key` declarations and validates pack-specific
  configuration. This two-phase approach allows workspaces to boot even when pack configuration is
  incomplete.
</Aside>

## Next Steps

- [Kernel Runtime](/kernel/runtime) — How the workspace spec is loaded and used
- [Packs](/kernel/packs) — Domain pack architecture and the Software Delivery Pack
- [Scope Intersection](/kernel/scope-intersection) — How workspace security becomes effective permissions
