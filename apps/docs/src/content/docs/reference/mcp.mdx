...
The MCP server uses a **pack dispatch** architecture: tools are executed through the [kernel runtime](/concepts/kernel), not by shelling out to CLI commands.
...
Every MCP tool call passes through the kernel's full execution pipeline, including [scope intersection](/concepts/scope-intersection), [policy evaluation](/concepts/policy-engine), and [evidence recording](/concepts/evidence). This means MCP tools receive the same security guarantees as direct kernel API calls.
...
3. **Kernel pipeline** — The tool passes through scope, policy, validation, and dispatch stages (see [Kernel Runtime](/concepts/kernel) for details)
4. **Evidence** — Success or denial is recorded in the [evidence store](/concepts/evidence) — the audit trail has no gaps
...
- [Kernel Runtime](/concepts/kernel) — How tool calls are authorized and executed
- [Packs](/concepts/packs) — Domain pack architecture (the Software Delivery Pack)
- [Evidence Store](/concepts/evidence) — How tool call evidence is recorded
