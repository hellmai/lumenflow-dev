...
The MCP server exposes **98 tools** organized into 12 categories:
...
| Additional WU Operations      | 16    | Extended WU management                     |
| Initiative Operations         | 8     | Multi-phase project orchestration          |
| Memory Operations             | 13    | Session tracking and agent coordination    |
...
| Spawn Operations              | 1     | Sub-agent spawn tree management            |
...
Public CLI parity target is **90 normalized public commands** (`:` and `-` converted to `_`).
Total MCP tool count is 98 because 8 tools are intentionally MCP-only extras:
...
| `lumenflow_gates`    | `lumenflow:gates`    | Framework gates wrapper         |
| `lumenflow_validate` | `lumenflow:validate` | Framework validation wrapper    |
...
Generate sub-agent spawn prompt for WU execution.
...
Monitor spawned agent progress and spawn health (stuck detection, zombie locks).
...
| Parameter | Type | Required | Description |
| ------------ | ------- | -------- | ---------------------------------------- |
| `wu` | string | No | WU ID to show spawns for (e.g., WU-1234) |
| `initiative` | string | No | Initiative ID to show all spawns for |
| `json` | boolean | No | Output as JSON |
...
<Aside type="note">
  Publish auth for `lumenflow_release` follows CLI `release` behavior: prefer `NPM_TOKEN` in
  automation, with support for `NODE_AUTH_TOKEN` or a `_authToken=` entry in `~/.npmrc` for local
  environments.
</Aside>
...
| `WU_SPAWN_ERROR`                 | Failed to spawn WU                   |
...
| `ORCHESTRATE_MONITOR_ERROR`      | Failed to monitor spawns             |
| `SPAWN_LIST_ERROR`               | Failed to list spawns                |
...
│   │
│   └── Setup Operations (8)
│       ├── lumenflow_init → CLI: lumenflow:init
