...
The LumenFlow MCP server exposes [Software Delivery Pack](/pack/overview) tools and kernel runtime tools to AI assistants via the Model Context Protocol. The majority of the 110 tools are Software Delivery Pack tools; 7 are kernel runtime task primitives for advanced use cases.
...
<Aside type="note">
  Runtime task tools are intentionally isolated from the legacy CLI-shell tools in `allTools`. This
  separation allows command-by-command migration to the kernel runtime without breaking parity
  gates. The production MCP server registers both arrays via `registeredTools`.
</Aside>
...
The MCP server uses a **pack dispatch** architecture: tools are executed through the [kernel runtime](/kernel/runtime), not by shelling out to CLI commands.
...
Every MCP tool call passes through the kernel's full execution pipeline, including [scope intersection](/kernel/scope-intersection), [policy evaluation](/kernel/policy-engine), and [evidence recording](/kernel/evidence-store). This means MCP tools receive the same security guarantees as direct kernel API calls.

<Aside type="note" title="Fallback behavior">
  If the kernel runtime fails to initialize (e.g., missing `workspace.yaml`), the MCP server falls
  back to CLI shell-out for compatibility. An allowlist controls which error codes trigger fallback
  — unknown errors are **not** eligible (fail-closed). Fallback events are recorded for telemetry.
</Aside>

### Execution Flow

1. **`runtimeFactory()`** — Creates a `KernelRuntime` instance for the workspace, loading the [workspace spec](/reference/workspace-spec) and resolving all pack pins
2. **`executeViaPack()`** — Routes the MCP tool call to the kernel's `executeTool()` method with the appropriate execution context
3. **Kernel pipeline** — The tool passes through scope, policy, validation, and dispatch stages (see [Kernel Runtime](/kernel/runtime) for details)
4. **Evidence** — Success or denial is recorded in the [evidence store](/kernel/evidence-store) — the audit trail has no gaps
...
- [Kernel Runtime](/kernel/runtime) — How tool calls are authorized and executed
- [Packs](/kernel/packs) — Domain pack architecture (the Software Delivery Pack)
- [Evidence Store](/kernel/evidence-store) — How tool call evidence is recorded
