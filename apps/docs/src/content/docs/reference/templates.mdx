...
description: Spawn prompt template format reference with YAML frontmatter
---

import { Aside, Tabs, TabItem } from '@astrojs/starlight/components';

Templates are markdown files with YAML frontmatter that define reusable prompt sections for `wu:spawn`. The template system enables customization of agent prompts without modifying core LumenFlow code.
...
When `wu:spawn` runs, templates are assembled in this order:
...
When running `wu:spawn --client claude`:
...
- [CLI Reference](/reference/cli) - `wu:spawn` command options
