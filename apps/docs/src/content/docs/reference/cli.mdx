...
<Aside type="note" title="Strict Validation (WU-1329)">
  WU validation is **strict by default**. Commands like `wu:create`, `wu:edit`, and `wu:validate`
  validate that code paths and test paths exist on disk. Use `--no-strict` to bypass (logged for
  audit). See individual command options for details.
</Aside>
...
Link a WU to an initiative bidirectionally. **Validates WU spec** before linking (WU-1330): rejects WUs with invalid schema, missing required fields, or bad paths.
...
Unlink a WU from an initiative bidirectionally
...
| -------------------------- | ------------------------------------- | -------- |
| `--initiative &lt;ref&gt;` | Parent initiative (INIT-XXX or slug)  | Yes      |
| `--wu &lt;wuId&gt;`        | Work Unit ID to unlink (e.g., WU-123) | Yes      |
...
| `--no-strict`                       | Bypass strict path validation (logged, not recommended)                           | No       |
...
Validate WU YAML files against schema. **Strict by default** (WU-1329): warnings are treated as errors.

```bash
pnpm wu:validate
pnpm wu:validate --id WU-123          # Validate specific WU (strict)
pnpm wu:validate --all --no-strict    # Validate all, warnings advisory
```
...
| ----------------------- | -------------------------------------------------------------------------------- | -------- |
| `-i, --id &lt;wuId&gt;` | Work Unit ID (e.g., WU-123). If not provided, auto-generates next sequential ID. | No       |
| `--all`                 | Validate all WU YAML files                                                       | No       |
| `--no-strict`           | Treat warnings as advisory (not errors). Bypasses strict mode.                   | No       |
