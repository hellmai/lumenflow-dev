...
| Initiatives     | [`initiative:add-wu`](#initiativeadd-wu), [`initiative:bulk-assign-wus`](#initiativebulk-assign-wus), [`initiative:bulk-assign`](#initiativebulk-assign), [`initiative:create`](#initiativecreate), [`initiative:edit`](#initiativeedit), [`initiative:list`](#initiativelist), [`initiative:plan`](#initiativeplan), [`initiative:status`](#initiativestatus)                                                                                                                                                                                                                                                                                                          |
...
| Other           | [`backlog:prune`](#backlogprune), [`file:delete`](#filedelete), [`file:edit`](#fileedit), [`file:read`](#fileread), [`file:write`](#filewrite), [`git:branch`](#gitbranch), [`git:diff`](#gitdiff), [`git:log`](#gitlog), [`git:status`](#gitstatus), [`init:plan`](#initplan), [`lane:health`](#lanehealth), [`lane:suggest`](#lanesuggest), [`plan:create`](#plancreate), [`plan:edit`](#planedit), [`plan:link`](#planlink), [`plan:promote`](#planpromote), [`signal:cleanup`](#signalcleanup), [`state:bootstrap`](#statebootstrap), [`state:cleanup`](#statecleanup), [`state:doctor`](#statedoctor), [`sync:templates`](#synctemplates), [`validate`](#validate) |
| Quality Gates   | [`gates`](#gates), [`gates:docs`](#gatesdocs)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Setup           | [`lumenflow`](#lumenflow), [`lumenflow:commands`](#lumenflowcommands), [`lumenflow:docs-sync`](#lumenflowdocs-sync), [`docs:sync`](#docssync), [`lumenflow:doctor`](#lumenflowdoctor), [`lumenflow:init`](#lumenflowinit), [`lumenflow:integrate`](#lumenflowintegrate), [`lumenflow:metrics`](#lumenflowmetrics), [`lumenflow:release`](#lumenflowrelease), [`lumenflow:sync-templates`](#lumenflowsync-templates), [`lumenflow:upgrade`](#lumenflowupgrade), [`lumenflow:validate`](#lumenflowvalidate)                                                                                                                                                               |
...
Link one or more WUs to an initiative bidirectionally
...
Create a checkpoint node for context snapshots. Also invoked automatically by auto-checkpoint hooks when `memory.enforcement.auto_checkpoint.enabled` is true (INIT-015).

```bash
pnpm mem:checkpoint
pnpm mem:checkpoint "Auto: sub-agent completed" --wu WU-123 --trigger subagent-stop --quiet
```
...
| `-t, --trigger &lt;type&gt;`  | Handoff trigger type (e.g., clear, handoff, subagent-stop) | No       |
| `-d, --base-dir &lt;path&gt;` | Base directory (defaults to current directory)             | No       |
| `-q, --quiet`                 | Suppress output except errors                              | No       |

**Auto-checkpoint hooks (INIT-015):** When configured, PostToolUse and SubagentStop hooks call `mem:checkpoint` automatically. PostToolUse uses a counter-based interval (default: every 30 tool calls); SubagentStop always checkpoints. See [Configuration](/reference/config#memoryenforcement) for setup.

### mem:cleanup

Prune closed memory nodes based on lifecycle policy, TTL, or decay scoring
...
When `--decay` is used, the command reads `memory.decay.threshold` and `memory.decay.half_life_days` from config (defaults: 0.1 and 30 respectively) and archives nodes whose decay score falls below the threshold. Combine with `--dry-run` to preview what would be archived.
...
Read coordination signals from other agents. Uses append-only receipts (INIT-015) to mark signals as read without rewriting `signals.jsonl`, preventing concurrent read-marking races between parallel agents.

```bash
pnpm mem:inbox
pnpm mem:inbox --no-mark   # Read without marking as read
pnpm mem:inbox --since 1h  # Signals from the last hour
```
...
| `--no-mark`                | Do not mark signals as read after display (no receipt appended)  | No       |
| `-c, --count`              | Output only signal count (lightweight polling for orchestrators) | No       |

**Signal receipts (INIT-015):** Read-marking appends receipt entries instead of modifying original signals. This prevents concurrent agents from overwriting each other's read state. Use `--no-mark` to read signals without generating receipts.
...
Generate recovery context for a WU (WU-1390). Outputs checkpoint data, recent signals, and session state to help a new agent resume work.
...
| Flag                          | Description                                    | Required |
| ----------------------------- | ---------------------------------------------- | -------- |
| `--wu &lt;wuId&gt;`           | Work Unit ID to recover (e.g., WU-123)         | Yes      |
| `-d, --base-dir &lt;path&gt;` | Base directory (defaults to current directory) | No       |
| `-q, --quiet`                 | Suppress output except recovery context        | No       |
...
Claim a work unit by creating a worktree/branch and updating status. On successful claim, surfaces any unread coordination signals from the memory layer so agents can see pending messages from other agents.
...
Complete a WU: run gates, fast-forward merge, stamp, and cleanup. Must be run from the main checkout (not a worktree). Use `wu:prep` first from the worktree.

With INIT-015 memory enforcement:

- **Checkpoint gate**: Warns or blocks if no checkpoints exist (configurable via `memory.enforcement.require_checkpoint_for_done`)
- **Decay archival**: Archives stale memory nodes when `memory.decay.enabled` and `trigger: on_done` (fail-open)
- **Signal read-marking**: Marks completed-WU signals as read using append-only receipts
- **Counter cleanup**: Removes auto-checkpoint counter files for the completed WU
