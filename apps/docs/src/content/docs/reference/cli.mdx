...
| Memory Layer    | [`mem:checkpoint`](#memcheckpoint), [`mem:cleanup`](#memcleanup), [`mem:create`](#memcreate), [`mem:export`](#memexport), [`mem:inbox`](#meminbox), [`mem:init`](#meminit), [`mem:ready`](#memready), [`mem:signal`](#memsignal), [`mem:start`](#memstart), [`mem:summarize`](#memsummarize), [`mem:triage`](#memtriage)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| Other           | [`backlog:prune`](#backlogprune), [`deps:add`](#depsadd), [`deps:remove`](#depsremove), [`file:delete`](#filedelete), [`file:edit`](#fileedit), [`file:read`](#fileread), [`file:write`](#filewrite), [`git:branch`](#gitbranch), [`git:diff`](#gitdiff), [`git:log`](#gitlog), [`git:status`](#gitstatus), [`guard:locked`](#guardlocked), [`guard:main:branch`](#guardmainbranch), [`guard:worktree:commit`](#guardworktreecommit), [`init:plan`](#initplan), [`lane:health`](#lanehealth), [`lane:suggest`](#lanesuggest), [`rotate:progress`](#rotateprogress), [`session:coordinator`](#sessioncoordinator), [`state:bootstrap`](#statebootstrap), [`sync:templates`](#synctemplates), [`trace:gen`](#tracegen), [`validate`](#validate), [`validate:agent:skills`](#validateagentskills), [`validate:agent:sync`](#validateagentsync), [`validate:backlog:sync`](#validatebacklogsync), [`validate:skills:spec`](#validateskillsspec) |
| Quality Gates   | [`gates`](#gates)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Setup           | [`lumenflow`](#lumenflow), [`lumenflow:docs:sync`](#lumenflowdocssync), [`lumenflow:doctor`](#lumenflowdoctor), [`lumenflow:init`](#lumenflowinit), [`lumenflow:metrics`](#lumenflowmetrics), [`lumenflow:release`](#lumenflowrelease), [`lumenflow:sync:templates`](#lumenflowsynctemplates), [`lumenflow:upgrade`](#lumenflowupgrade), [`lumenflow:validate`](#lumenflowvalidate)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
Check lane configuration health - detects overlapping code_paths between lanes and identifies files not covered by any lane definition.
...
| Flag            | Description                         | Required |
| --------------- | ----------------------------------- | -------- |
| `-j, --json`    | Output results as JSON              | No       |
| `-v, --verbose` | Show verbose output with all files  | No       |
| `--no-coverage` | Skip coverage gap detection         | No       |
...
============================================================
  ...
  All lane configurations are valid:
    - No overlapping code_paths detected
    - All code files covered by lanes
...
- `0` - Lane configuration is healthy (no overlaps or gaps)
- `1` - Issues detected (overlapping paths or coverage gaps)

**Integration:**

Lane health is also checked by:
- `lumenflow doctor` - Includes lane health in system health check
- `pnpm gates` - Configurable via `gates.lane_health` in `.lumenflow.config.yaml`

### lane:suggest

LLM-driven lane suggestions based on codebase context. Analyzes project structure, package layout, and optionally git history to suggest appropriate lane definitions.
...
| Flag                   | Description                                           | Required |
| ---------------------- | ----------------------------------------------------- | -------- |
| `--dry-run`            | Show what would be suggested without LLM call         | No       |
| `-i, --interactive`    | Interactively accept/skip/edit each suggestion        | No       |
| `-o, --output <file>`  | Write suggestions to YAML file                        | No       |
| `--json`               | Output suggestions as JSON                            | No       |
| `--no-llm`             | Use heuristic-based suggestions (no LLM)              | No       |
| `--include-git`        | Add git history context (co-occurrence, ownership, churn) | No       |

**Examples:**

```bash
# Preview suggestions without LLM call
pnpm lane:suggest --dry-run

# Generate and save to file
pnpm lane:suggest --output .lumenflow.lane-inference.yaml

# Interactive mode to review each suggestion
pnpm lane:suggest --interactive

# Include git history for better context
pnpm lane:suggest --include-git
```

**Interactive Mode:**

In interactive mode, for each suggestion you can:
- `a` (accept) - Add suggestion to final output
- `s` (skip) - Skip this suggestion
- `e` (edit) - Modify the lane name or description
- `q` (quit) - Stop and return accepted suggestions

**Git History Context (`--include-git`):**

When enabled, extracts insights from git history:
- **Co-occurrence**: Files frequently changed together (suggests shared ownership)
- **Ownership signals**: Primary contributors for different areas
- **Churn hotspots**: Files with high change frequency (potential complexity)
