...
<Aside type="tip">
  This page lists primary commands only. Use `pnpm lumenflow:commands` for full command discovery
  including aliases and legacy entrypoints.
</Aside>
...
| Category            | Commands                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gates & Quality     | [`gates`](#gates), [`lane:create`](#lanecreate), [`lane:edit`](#laneedit), [`lane:health`](#lanehealth), [`lane:lock`](#lanelock), [`lane:setup`](#lanesetup), [`lane:status`](#lanestatus), [`lane:suggest`](#lanesuggest), [`lane:validate`](#lanevalidate), [`lumenflow:pre-commit-check`](#lumenflowpre-commit-check), [`validate`](#validate)                                                                                                                                                                                                                                                                                                                                                    |
...
| State Management    | [`state:bootstrap`](#statebootstrap), [`state:cleanup`](#statecleanup), [`state:doctor`](#statedoctor), [`state:emit`](#stateemit)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| WU Lifecycle        | [`wu:block`](#wublock), [`wu:brief`](#wubrief), [`wu:claim`](#wuclaim), [`wu:create`](#wucreate), [`wu:delegate`](#wudelegate), [`wu:done`](#wudone), [`wu:edit`](#wuedit), [`wu:escalate`](#wuescalate), [`wu:prep`](#wuprep), [`wu:recover`](#wurecover), [`wu:release`](#wurelease), [`wu:sandbox`](#wusandbox), [`wu:status`](#wustatus), [`wu:unblock`](#wuunblock), [`wu:validate`](#wuvalidate)                                                                                                                                                                                                                                                                                                |
...
```bash
pnpm wu:brief --id <wuId> --client <client>
```

**Behavior:**

- Records `wu:brief` evidence for lifecycle policy checks.
- Does not record delegation lineage intent. Use `wu:delegate --parent-wu <P>` for lineage.
- `--evidence-only` records evidence without prompt output for self-implementation.
- In non-claimed checkouts/branches, prompt generation remains available but evidence writes are skipped.
...
| `--evidence-only`          | Record `wu:brief` evidence only (no handoff prompt output)                                        | No       |
| `--no-context`             | Skip memory context injection in handoff prompts                                                  | No       |
| `--thinking`               | Enable extended thinking for sub-agent execution                                                  | No       |
...
| `--strict-sizing`          | Block when `sizing_estimate` metadata is missing or exceeds thresholds without exception metadata | No       |
| `--parent-wu &lt;wuId&gt;` | Parent WU context hint for orchestration (does not record lineage intent in `wu:brief`)           | No       |
| `--codex`                  | Deprecated alias for `--client codex-cli`                                                         | No       |
...
**Claim-time auto-brief policy:**

`wu:claim` can auto-run `wu:brief` evidence recording based on `wu.brief.policyMode` (`off`,
`manual`, `auto`, `required`). Claim-time auto-brief records evidence only and does not emit a
handoff prompt.
...
Generate delegation prompt and record explicit lineage intent plus brief attestation metadata.
...
- Parent and target WU lineage
- Delegation status/timestamps
- Brief attestation (`sha256` prompt hash, prompt length, generated timestamp, client)

For initiative-governed and explicitly delegated WUs, `wu:done` validates claim pickup evidence and
matching attested `wu:brief` hash before completion. Use `--force` only for audited legacy
overrides.
