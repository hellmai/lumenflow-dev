...
| `--integrity &lt;hash&gt;`   | Expected SHA-256 integrity hash in "sha256:<hex>" format (required for source: registry) | No       |
...
| Flag                          | Description                                            | Required |
| ----------------------------- | ------------------------------------------------------ | -------- |
| `--yes, -y`                   | Accept all defaults non-interactively                  | No       |
| `--domain &lt;domain&gt;`     | Domain pack: software-delivery, infra, or custom       | No       |
| `--project-name &lt;name&gt;` | Project name (default: directory name or "my-project") | No       |
| `--output, -o &lt;dir&gt;`    | Output directory (default: current directory)          | No       |
| `--force, -f`                 | Overwrite existing workspace.yaml                      | No       |
| `--skip-pack-install`         | Skip pack installation step                            | No       |
| `--skip-dashboard`            | Skip dashboard launch step                             | No       |
