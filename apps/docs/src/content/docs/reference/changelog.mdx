...
- **Published stable alignment**: Current package line for `@lumenflow/*` is `v3.9.6`.
- **Language guide install pin refresh**: Pack-scoped quickstarts now use `npx @lumenflow/cli@3.9.6`.
- **Reference regeneration**: Public CLI/config reference docs regenerated from current command/config schema.
...
- **Kernel + Pack docs split**: Public docs now separate kernel-level architecture from Software Delivery Pack usage docs.
- **Pack-scoped language guides**: Language guides moved under Software Delivery Pack and standardized to verified quickstart sections.
- **Curated CLI surface docs**: CLI reference generation now prefers primary command surface and excludes alias/legacy noise from default tables.
- **Version policy explicit**: User-facing latest claims are anchored to published stable release metadata.
...
- **Kernel + Pack architecture**: Domain-agnostic kernel runtime with pluggable packs. The Software Delivery Pack provides 90+ tools; additional domains ship as first-class packs using the same manifest/runtime contract.
- **`workspace.yaml`**: Unified workspace configuration. Top-level fields define kernel runtime (packs, lanes, security, namespaces), and `software_delivery` configures CLI/tooling behavior.
- **14 granular lanes**: Framework lanes split into Core Lifecycle, Core Validation, Core State Recovery, CLI WU Commands, CLI Memory State, CLI Orchestration, CLI Enforcement, plus package-specific lanes. Variable WIP limits (1 or 2) with `lock_policy` and `wip_justification` fields.
- **Runtime API migration complete (INIT-030)**: Zero remaining shell-outs in MCP tools — all tool implementations now use runtime APIs directly.
- **New packages**: `@lumenflow/kernel`, `@lumenflow/runtime`, `@lumenflow/packs`, `@lumenflow/mcp`, `@lumenflow/control-plane-sdk` join the existing package family.
...
See [Migration Guide](/packs/software-delivery/setup/migration-guide) for upgrading from v2.x.
...
- **Methodology configuration**: Configure testing (`tdd`, `test-after`, `none`) and architecture (`hexagonal`, `layered`, `none`) methodology in `workspace.yaml`
...
No migration required. See [Choosing Your Methodology](/packs/software-delivery/workflows/choosing-methodology) for guidance on adopting methodology configurability.
...
See the [v2.2.0 Release Notes](/releases/v220) for complete details.
...
- **Config version bump**: `workspace.yaml` now requires `version: '2.0'`
...
See [Migration Guide](/packs/software-delivery/setup/migration-guide) for upgrading from v1.x.
...
Current package versions as of v3.0.0:

| Package                        | Version | Description                           |
| ------------------------------ | ------- | ------------------------------------- |
| `@lumenflow/core`              | 3.0.0   | Core WU lifecycle and state machine   |
| `@lumenflow/cli`               | 3.0.0   | Command-line interface                |
| `@lumenflow/kernel`            | 3.0.0   | Domain-agnostic runtime primitives    |
| `@lumenflow/runtime`           | 3.0.0   | Runtime daemon, scheduler, sessions   |
| `@lumenflow/packs`             | 3.0.0   | Pack framework (kernel dispatch)      |
| `@lumenflow/mcp`               | 3.0.0   | MCP server and tool definitions       |
| `@lumenflow/memory`            | 3.0.0   | Session tracking and context recovery |
| `@lumenflow/agent`             | 3.0.0   | Agent definitions and skill loading   |
| `@lumenflow/initiatives`       | 3.0.0   | Multi-phase project orchestration     |
| `@lumenflow/metrics`           | 3.0.0   | DORA metrics and flow analysis        |
| `@lumenflow/shims`             | 3.0.0   | Git and pnpm safety shims             |
| `@lumenflow/control-plane-sdk` | 3.0.0   | SDK for control plane integration     |
...
- Single-word lane names — All lanes must use "Parent: Sublane" format
- Config `version: '1.0'` — Must use `version: '2.0'`
