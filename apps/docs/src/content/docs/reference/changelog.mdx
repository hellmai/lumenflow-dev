...
- **Kernel + Pack architecture**: Domain-agnostic kernel runtime with pluggable packs. The Software Delivery Pack provides 90+ tools; future packs (compliance-audit, infra-ops, security-ops) are scaffolded.
- **`workspace.yaml`**: New kernel-level configuration file defining packs, lanes, security boundaries, and namespaces — distinct from `.lumenflow.config.yaml` (CLI/tooling layer).
