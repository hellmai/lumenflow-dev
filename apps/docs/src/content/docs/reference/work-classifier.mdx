...
Extend default patterns via `methodology.work_classification` in `workspace.yaml`.
...
The classifier emits methodology hints as recommendations for prompt composition. They are not a
hardcoded software-engineering mandate.

### `smoke-test`

When the detected domain is `ui` and confidence >= 0.5, the classifier returns
...
When every declared code path is structured content (`.yaml`, `.yml`, `.json`, `.md`, `.mdx`),
...
The classifier also feeds prompt-template conditions such as `work.domain` and
