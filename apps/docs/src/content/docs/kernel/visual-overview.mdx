...
<Aside type="note" title="Architecture layers">
  ...
  diagram shows the domain-agnostic [kernel](/kernel/runtime). The remaining diagrams (WU
  Lifecycle, Worktree Discipline, Gates Pipeline, Lane WIP, Memory) show concepts from the [Software
  Delivery Pack](/packs/software-delivery/overview) that build on top of the kernel.
</Aside>
