...
The kernel is domain-agnostic. It does not know about Work Units, lanes, or gates. Those concepts come from [packs](/kernel/packs). The kernel provides four capabilities that any pack can build on:

1. [Scope intersection](/kernel/scope-intersection) — permission enforcement
2. [Policy evaluation](/kernel/policy-engine) — deny-wins rule cascade
3. [Evidence recording](/kernel/evidence-store) — immutable audit trail
...
2. **Scope resolution.** The kernel reads four sets of allowed scopes — workspace, lane, task, and tool — and computes their [intersection](/kernel/scope-intersection). If the intersection is empty, the call fails with `SCOPE_DENIED`.
...
5. **Policy evaluation.** The kernel's [policy engine](/kernel/policy-engine) evaluates all matching rules with the `on_tool_request` trigger. A single deny from any layer makes the decision final.
...
7. **Dispatch.** The kernel routes the call to the tool's handler — either an in-process function or a sandboxed subprocess. For details on what happens inside dispatch — including sandbox execution, the tool-runner worker, and the runtime CLI adapter pattern — see [Tool Execution](/kernel/tool-execution).

8. **Evidence recording.** The result (success, failure, denied, or crashed) is appended to the [evidence store](/kernel/evidence-store) with timing, scope enforcement details, and policy decisions.
...
2. All [packs](/kernel/packs) referenced in the workspace spec are loaded, validated, and their tools registered.
3. The [policy engine](/kernel/policy-engine) is constructed with a four-layer stack built from pack-declared policies.
4. The [evidence store](/kernel/evidence-store) is initialized and any orphaned traces from a previous crash are reconciled.
...
- [Scope Intersection](/kernel/scope-intersection) — How the 4-level permission model works
- [Policy Engine](/kernel/policy-engine) — How deny-wins evaluation works
- [Evidence Store](/kernel/evidence-store) — How the audit trail is built
- [Packs](/kernel/packs) — How tools are declared and loaded
