...
Events are written to `.beacon/telemetry/` as JSONL files. Integrate with your observability stack:

```bash
# Stream to Datadog
tail -f .beacon/telemetry/events.jsonl | datadog-agent

# Export to Prometheus
pnpm metrics:export --format prometheus
```
