...
- **`workspace.yaml`**: New kernel-level config file (distinct from `.lumenflow.config.yaml`). Defines packs, lanes, security boundaries, and namespaces. See [Workspace Spec](/reference/workspace-spec).
...
# Update .lumenflow.config.yaml lane definitions
...
<Aside type="tip">
  Lane granularity is optional. Legacy lane names (`Framework: Core`, `Framework: CLI`, etc.)
  are supported via compatibility definitions. Adopt granular lanes when ready.
</Aside>
