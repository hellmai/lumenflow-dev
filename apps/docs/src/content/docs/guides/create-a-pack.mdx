...
<Aside type="tip">
  ...
  `config_key: customer_support`, making `customer_support.routing_rules` a valid config path.
  Each pack's config key must be unique across all packs pinned in the workspace.
</Aside>
