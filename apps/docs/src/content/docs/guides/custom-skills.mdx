...
<Aside type="tip">
  Most skills only need `Read, Bash, Grep`. Add `Write` and `Edit` only if the skill involves file creation or modification.
</Aside>
...
- Read the [Skills Index](/reference/skills) for built-in skills
