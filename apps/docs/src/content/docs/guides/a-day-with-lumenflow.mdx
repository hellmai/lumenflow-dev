...
<Tabs syncKey="workflow">
  <TabItem label="AI-Assisted">
    ```bash
    # AI agent starts by checking memory
    pnpm mem:ready
    # Output: 2 WUs ready in your lanes

    # Review backlog
    pnpm wu:list --status ready --lane "Framework: Core"
    ```
  </TabItem>
...
<Aside type="caution">
  Always `cd` into the worktree immediately after claiming. Working in the main checkout will cause hook failures and state inconsistencies.
</Aside>
...
<Aside type="tip">
  This confirms our tests actually test something. If tests pass before implementation, they're not testing the right thing.
</Aside>
...
<Tabs syncKey="workflow">
  <TabItem label="AI-Assisted">
    ...
    # Output:
    # [gates] Running format check...
    # [gates] Running lint...
    # [gates] Running typecheck...
    # [gates] Running tests...
...
<Tabs syncKey="workflow">
  <TabItem label="AI-Assisted">
    ...
    pnpm mem:signal "Implementation complete, gates passing"
    ...
    # Output:
    # [wu:done] Running gates in worktree...
    # [wu:done] Gates passed
...
<Aside type="caution">
  Always run `wu:done` from the main checkout, not from within the worktree.
</Aside>
...
|------|----------|----------|
| 9:00 | Review ready WUs | `pnpm wu:list --status ready` |
...
| 15:00 | Run gates | `pnpm gates` |
| 15:30 | Commit changes | `git commit` |
| 16:00 | Complete WU | `pnpm wu:done --id WU-042` |
...
# If in main, stash and apply to worktree
git stash
cd worktrees/framework-core-wu-042
git stash pop
```
...
<CardGrid>
  ...
  <Card title="Gates before done" icon="approve-check">
    Run `pnpm gates` before `wu:done` to catch issues early.
  </Card>
