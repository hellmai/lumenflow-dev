...
<Aside type="danger">
  Only use when failures are pre-existing, not caused by your changes.
</Aside>
...
# Signal progress
pnpm mem:signal "Tests written, starting implementation"
...
```bash
# All in progress
pnpm wu:list --status in_progress

# By lane
pnpm wu:list --lane "Framework: Core"
```
...
| Task | Command |
|------|---------|
| Create WU | `pnpm wu:create --id WU-XXX ...` |
...
| Run gates | `pnpm gates` |
| Fix format | `pnpm format` |
| Run tests | `pnpm test` |
| Memory checkpoint | `pnpm mem:checkpoint` |
| Check ready work | `pnpm mem:ready` |
