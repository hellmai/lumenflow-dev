...
| Production-critical regulated/fintech code | `tdd` with 95% threshold | Maximum safety for regulated environments |
...
5. **Communicate the change**
   ...
   `wu:done` but not before implementation.
...
6. **Plan return to TDD**
   ...
   methodology:
     testing: 'test-after'
     # TODO: Review at end of Q2 - consider returning to TDD
   ```

</Steps>

<Aside type="tip">
  Use `test-after` as a stepping stone, not a permanent destination. Once legacy code has better coverage, consider returning to TDD.
</Aside>
...
<Aside type="tip">
  If the prototype succeeds and becomes production code, migrate to `test-after` immediately. Don't ship prototype code without a plan for tests.
</Aside>
