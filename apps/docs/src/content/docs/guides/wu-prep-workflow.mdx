...
```bash
pnpm wu:prep --id WU-123 [--docs-only]
```
...
If you see "wu:done must be run from main checkout", you ran wu:done from a worktree. Use `wu:prep` instead, then follow its copy-paste instruction.
</Aside>
