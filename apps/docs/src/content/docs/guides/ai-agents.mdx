...
Skills are loaded automatically by `wu:spawn` based on WU context, or manually:
...
Use `wu:spawn` to generate Task tool invocations with proper context:

```bash
pnpm wu:spawn --id WU-001 --client claude-code
```
