...
- Node.js 22+ (for CLI) or skip CLI and use GitHub App only
...
2. **Initialize LumenFlow**
   ...
   **Using a specific AI coding tool?** Use the `--client` flag to generate client-specific overlay files:
   ...
   npx lumenflow-init --client claude

   # Cursor IDE
   npx lumenflow-init --client cursor

   # Windsurf IDE
   npx lumenflow-init --client windsurf

   # All clients
   npx lumenflow-init --client all
   ```
...
3. **Review generated files**
   ...
   <FileTree>
     - .lumenflow.config.yaml (your configuration) - LUMENFLOW.md (workflow entry point) - AGENTS.md
...
npx wu-claim --id WU-123 --lane "Operations: Infrastructure"
