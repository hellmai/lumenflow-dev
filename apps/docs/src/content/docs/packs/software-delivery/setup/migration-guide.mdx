...
- **`workspace.yaml`**: Kernel-level workspace spec plus CLI/tooling config in `software_delivery`. Defines packs, lanes, security boundaries, namespaces, and delivery workflow settings. See [Workspace Spec](/reference/workspace-spec).
...
<Aside type="tip">
  Lane granularity is optional. Legacy lane names (`Framework: Core`, `Framework: CLI`, etc.) are
  supported via compatibility definitions. Adopt granular lanes when ready.
</Aside>
...
3. **Set up LumenFlow**
   ...
   Follow the [Existing Projects Guide](/packs/software-delivery/setup/existing-projects) to set up LumenFlow in your repo.
...
9. **Team onboarding**
   ...
   - Share the [Manual Quickstart (Humans)](/getting-started/manual-quickstart) if teammates run setup themselves
...
- [Existing Projects](/packs/software-delivery/setup/existing-projects) - Initial setup
- [Solo Developer Workflow](/packs/software-delivery/workflows/solo-workflow) - Simpler setup for individuals
- [Team Workflow](/packs/software-delivery/workflows/team-workflow) - Team practices
- [Initiatives](/packs/software-delivery/advanced/initiatives) - Multi-WU planning
