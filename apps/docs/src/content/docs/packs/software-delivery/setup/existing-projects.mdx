...
<Aside type="note" title="Software Delivery Pack setup">
  This guide walks through adding the [Software Delivery Pack](/packs/software-delivery/overview) to
  an existing project. The pack provides WUs, lanes, gates, and all CLI commands. The underlying
  [kernel](/kernel/runtime) handles governance (scope, policy, evidence) automatically.
</Aside>
...
<Aside type="note" title="Path configuration is workspace-driven">
  Task and docs paths are resolved from `workspace.yaml` (`software_delivery.directories.*`). The
  default layout is `docs/tasks/*`. If you prefer arc42, run init with `--docs-structure arc42` or
  set directory paths explicitly.
</Aside>
...
- Node.js 22+ (for CLI)
- Understanding of [Work Units](/packs/software-delivery/concepts/work-units) and [Gates](/packs/software-delivery/concepts/gates)
...
3. **Review generated files**
   ...
   <FileTree>
     ...
     agents/ (AI agent guidance) - docs/tasks/ - backlog.md - status.md - wu/ (WU spec directory) -
     _frameworks/lumenflow/agent/onboarding/ - quick-ref-commands.md (CLI command reference) -
     first-wu-mistakes.md (common pitfalls) - troubleshooting-wu-done.md (wu:done issues) -
     agent-safety-card.md (safety reference)
   </FileTree>
...
1. **Create configuration file**
   ...
   software_delivery:
     directories:
       ...
       initiatives_dir: docs/tasks/initiatives
       backlog_path: docs/tasks/backlog.md
       status_path: docs/tasks/status.md
     state:
...
2. **Create LUMENFLOW.md**
   ...
   1. Review backlog: `docs/tasks/backlog.md`
   2. Create WU: Create YAML in `docs/tasks/wu/`
...
3. **Bulk migrate** - Script conversion (see [Migration Guide](/packs/software-delivery/setup/migration-guide))
...
- [Migration Guide](/packs/software-delivery/setup/migration-guide) - Migrate from existing tools
- [Solo Developer Workflow](/packs/software-delivery/workflows/solo-workflow) - Simpler setup for individuals
- [Team Workflow](/packs/software-delivery/workflows/team-workflow) - Team-specific practices
