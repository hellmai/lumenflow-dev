...
<Aside type="tip">
  Use the narrowest scope that works. A read-only tool should never declare `access: write`.
  For network access, prefer `posture: allowlist` over `posture: full` whenever possible --
  list only the hosts the tool actually needs. The kernel enforces the intersection, so overly
  broad scopes are capped by the workspace and lane configuration.
</Aside>
...
If the workspace or lane declares a narrower allowlist, the kernel intersects the entries -- only hosts present in _all_ layers are permitted. If the workspace blocks network entirely (`posture: off`), the tool call is denied regardless of the tool's declared allowlist.
...
- [Gates](/packs/software-delivery/concepts/gates) -- How gate policies work end-to-end
- [Constraints](/packs/software-delivery/concepts/constraints) -- The rules packs must follow
