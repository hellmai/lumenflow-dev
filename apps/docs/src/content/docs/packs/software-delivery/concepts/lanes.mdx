...
<Aside type="note" title="Software Delivery Pack concept">
  Lanes are provided by the [Software Delivery Pack](/packs/software-delivery/overview). At the
  kernel level, lanes define **scope boundaries** — one of four levels in the [scope
  intersection](/kernel/scope-intersection) algorithm. Other [packs](/kernel/packs) can define their
  own scoping mechanisms that feed into the same kernel intersection.
</Aside>

Lanes are parallel work streams that organize work by domain and prevent overload through WIP (Work-in-Progress) limits. At the kernel level, lanes also define **scope boundaries** — the [scope intersection](/kernel/scope-intersection) algorithm uses lane `code_paths` as one of four permission levels that determine what an agent can access.
...
Lane `code_paths` serve a dual purpose: they organize work AND enforce security boundaries through [scope intersection](/kernel/scope-intersection).

When a tool call executes, the [kernel](/kernel/runtime) computes:
...
<Aside type="note">
  ...
  applies. See [Scope Intersection](/kernel/scope-intersection) for the full algorithm.
</Aside>
...
## Suggestions via Paths

LumenFlow suggests lanes based on the authoritative lane definitions in
`workspace.yaml`:
...
If a WU's `code_paths` match configured lane `code_paths`, the lane is suggested
automatically by `wu:infer-lane`.
...
- [Scope Intersection](/kernel/scope-intersection) — How lane paths become permission boundaries
- [Policy Engine](/kernel/policy-engine) — Lane-level policy rules
- [Gates](/packs/software-delivery/concepts/gates) — Quality checks per lane
- [Team Workflow](/packs/software-delivery/workflows/team-workflow) — Lane strategies for teams
