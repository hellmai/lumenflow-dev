...
<Aside type="note" title="Software Delivery Pack concept">
  Work Units are provided by the [Software Delivery Pack](/packs/software-delivery/overview). At the
  kernel level, WUs map to **tasks** — the kernel's generic lifecycle primitive. Other
  [packs](/kernel/packs) can model their own domain objects (infrastructure change requests,
  compliance reviews) using the same kernel [task lifecycle](/kernel/runtime).
</Aside>
...
See [Sizing Guide](/packs/software-delivery/workflows/sizing) for detailed heuristics.
...
Every tool call during a WU's execution is recorded in the [evidence store](/kernel/evidence-store):

- **Content-addressed inputs** — Tool inputs are hashed with SHA-256 and stored as immutable blobs
- **Tool traces** — `TOOL_CALL_STARTED` and `TOOL_CALL_FINISHED` entries with timing, scope enforcement results, and [policy decisions](/kernel/policy-engine)
...
This audit trail answers: _What was the agent asked to do? Was the action authorized? What actually happened?_ See [Evidence Store](/kernel/evidence-store) for the full model.
...
- [Lanes](/packs/software-delivery/concepts/lanes) — Organizing work streams with scope boundaries
- [Gates](/packs/software-delivery/concepts/gates) — Quality enforcement as kernel policies
- [Evidence Store](/kernel/evidence-store) — The immutable audit trail behind every WU
- [Kernel Runtime](/kernel/runtime) — How WU tasks map to kernel task lifecycle
- [Sizing Guide](/packs/software-delivery/workflows/sizing) — How to size WUs properly
