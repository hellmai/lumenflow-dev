...
<Aside type="note" title="Software Delivery Pack feature">
  The Memory Layer is provided by the [Software Delivery Pack](/packs/software-delivery/overview). Memory tools
  (`mem:checkpoint`, `mem:signal`, `mem:inbox`) are pack tools that produce evidence through the
