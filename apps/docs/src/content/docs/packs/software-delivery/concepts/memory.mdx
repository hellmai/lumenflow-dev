...
<Aside type="note" title="Software Delivery Pack feature">
  The Memory Layer is provided by the [Software Delivery Pack](/packs/software-delivery/overview).
  Memory tools (`mem:checkpoint`, `mem:signal`, `mem:inbox`) are pack tools that produce evidence
  through the [kernel](/kernel/runtime).
</Aside>
...
<Aside type="note">
  Configure `progress_signals.enabled: true` in `workspace.yaml` to make signaling mandatory for
  sub-agents. See [Progress Signals](#progress-signals) for all options.
</Aside>
...
- [AI Agent Integration](/packs/software-delivery/agents/ai-agent-integration) - Using memory with agents
- [Agent Onboarding](/packs/software-delivery/agents/agent-onboarding) - Getting started
