...
<Aside type="note" title="Gates are kernel-enforced policies">
  In the [Software Delivery Pack](/kernel/packs), gates are implemented as pack-layer policies
  with `trigger: on_completion`. The [policy engine](/kernel/policy-engine) evaluates them when a
