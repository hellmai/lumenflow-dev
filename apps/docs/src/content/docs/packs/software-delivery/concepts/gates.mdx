...
<Aside type="note" title="Gates are kernel-enforced policies">
  In the [Software Delivery Pack](/kernel/packs), gates are implemented as pack-layer policies with
  `trigger: on_completion`. The [policy engine](/kernel/policy-engine) evaluates them when a task
  tries to complete — an agent cannot skip gates by calling `completeTask` directly.
</Aside>
...
When `wu:prep` or `wu:done` runs, the [kernel](/kernel/runtime) evaluates these policies. A `deny` from any gate makes the completion decision final — the [deny-wins invariant](/kernel/policy-engine#deny-wins-evaluation) applies. The result is recorded in the [evidence store](/kernel/evidence-store) for audit.
...
- [Policy Engine](/kernel/policy-engine) — How gates are evaluated as deny-wins policies
- [Evidence Store](/kernel/evidence-store) — How gate results are recorded for audit
