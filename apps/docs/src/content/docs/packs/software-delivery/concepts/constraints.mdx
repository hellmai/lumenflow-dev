...
<Aside type="note" title="Software Delivery Pack constraints">
  These constraints are defined by the [Software Delivery Pack](/packs/software-delivery/overview).
  They govern software delivery workflows specifically. The [kernel](/kernel/runtime) provides the
  underlying enforcement mechanisms (scope intersection, policy engine, evidence recording) that
  make these constraints inescapable. Other [packs](/kernel/packs) define their own domain-specific
  constraints.
</Aside>
