...
<Aside type="note" title="Software Delivery Pack workflow">
  This guide covers team workflows using the [Software Delivery
  Pack](/packs/software-delivery/overview). Lane strategies, claiming protocols, and WIP enforcement
  are pack concepts enforced by the [kernel](/kernel/runtime).
</Aside>
...
When an initiative spans multiple WUs, orchestrate agents across lanes:

```bash
# Launch initiative orchestration
pnpm orchestrate:initiative --id INIT-007

# Monitor agent activity
pnpm orchestrate:monitor

# Check coordination signals between agents
pnpm mem:inbox --since 30m
```

Agents coordinate via the [memory layer](/packs/software-delivery/concepts/memory) using signals and checkpoints. Each agent works in its own worktree, isolated from other agents.

<Aside type="tip">
  Start with `wu:brief` for simple delegation. Use `wu:delegate` when you need lineage tracking. Use
  `orchestrate:initiative` when coordinating 3+ agents on an initiative.
</Aside>

## Next Steps

- [AI Agent Integration](/packs/software-delivery/agents/ai-agent-integration) -- Working with AI teammates
- [Initiatives](/packs/software-delivery/advanced/initiatives) -- Multi-phase project coordination
- [Memory Layer](/packs/software-delivery/concepts/memory) -- Context preservation and agent coordination
- [CLI Reference](/reference/cli) -- Team commands
