...
| Production-critical regulated code         | `tdd` with 95% threshold | Maximum safety for regulated environments   |
...
1. **Assess current state**
   ...
   cat workspace.yaml | grep -A 5 methodology
   ```
...
1. Check agent prompts include methodology:
   ...
   ```bash
   pnpm wu:brief --id WU-042 --client <client> | grep -i methodology
   ```
...
3. Verify `workspace.yaml` is readable:
   ```bash
...
```
ERROR: workspace.yaml validation failed
...
git checkout HEAD~1 -- workspace.yaml
...
- [Choosing Your Methodology](/packs/software-delivery/workflows/choosing-methodology) - Understand the tradeoffs
- [Configuration Reference](/reference/config) - All configuration options
- [Existing Projects](/packs/software-delivery/setup/existing-projects) - Initial LumenFlow setup
- [Troubleshooting](/packs/software-delivery/setup/troubleshooting) - General troubleshooting
