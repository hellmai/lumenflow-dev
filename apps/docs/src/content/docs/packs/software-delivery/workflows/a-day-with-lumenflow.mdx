...
<Aside type="note" title="Software Delivery Pack workflow">
  This walkthrough uses the [Software Delivery Pack](/packs/software-delivery/overview) —
  LumenFlow's built-in pack for software development. The CLI commands (`wu:claim`, `wu:prep`,
  `wu:done`, `gates`) and concepts (WUs, lanes, worktrees) are all pack-provided tools that route
  through the [kernel](/kernel/runtime) for governance.
</Aside>
