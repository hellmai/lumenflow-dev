...
Each event is a JSON object written as a single line to `.lumenflow/telemetry/events.jsonl`:
...
```json
{
  ...
  "agent": "claude-code",
  "metadata": {
    "gate_results": { "format": "pass", "lint": "pass", "typecheck": "pass", "test": "pass" },
    "files_changed": 5
  }
}
```
...
```bash
# Stream to stdout (for piping to external tools)
tail -f .lumenflow/telemetry/events.jsonl

# Filter for failures only
grep '"gate.failed"' .lumenflow/telemetry/events.jsonl

# Generate metrics snapshot for dashboards
pnpm metrics:snapshot --format json > metrics.json

# Count events by type
cat .lumenflow/telemetry/events.jsonl | jq -r '.event' | sort | uniq -c | sort -rn
```

### External Tool Integration

For teams using observability platforms:

- **Datadog/New Relic:** Pipe JSONL events via their log agents or use `metrics:snapshot --format json` with their API
- **Grafana:** Export snapshots as JSON and import into Grafana dashboards
- **CI/CD:** Run `pnpm flow:report --format json` in your pipeline to track trends over time
...
- [Team Workflow](/packs/software-delivery/workflows/team-workflow) – Team practices and conventions
