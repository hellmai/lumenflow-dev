...
`wu:brief` generates a complete context-loaded prompt for a sub-agent and records `wu:brief`
execution evidence for lifecycle policy checks:
...
Use `wu:brief` when you want to review the prompt before spawning and you do not need delegation
lineage intent recording.
...
- Brief attestation (`sha256` prompt hash, prompt length, generated timestamp)
...
- Pickup evidence exists (claim handshake)
- Brief attestation exists and hash matches recorded `wu:brief` evidence
...
- [Flow Metrics](/packs/software-delivery/advanced/metrics) -- Track delivery performance
- [AI Agents](/packs/software-delivery/agents/ai-agent-integration) -- Agent orchestration
- [Work Units](/packs/software-delivery/concepts/work-units) -- WU fundamentals
