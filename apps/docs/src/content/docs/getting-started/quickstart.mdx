...
If the safe-git file is missing, stop and ask for direction. Do not guess or search for alternate paths.

Re-run `pnpm exec lumenflow-doctor`. If errors remain, stop and ask for direction.

Lane overlap warnings are not fixed during setup. Create a follow-up WU later. Do not edit
`.lumenflow.config.yaml` during setup.
...
| `pnpm exec lumenflow-integrate --client claude-code`           | Claude Code only: generate enforcement hooks |
...
5.  **Initialize LumenFlow**
    ...
    ```
        ...
        <Aside type="tip" title="Claude Code enforcement hooks (optional)">

    If the user is on Claude Code and wants enforcement hooks that block edits outside worktrees:

    ```bash
    pnpm exec lumenflow-integrate --client claude-code
    ```

    This generates `.claude/hooks/*` and updates `settings.json`. Other clients don't support this yet.
...
3.  **Initialize with merge mode**
    ...
    If the user is on Claude Code and wants enforcement hooks:
...
7.  **Create the first WU (do not implement)**
    ...
    If the user shares a product/vision document during setup, **do not** save
    it to the repo yet. Create a Discovery WU titled "Ingest product document"
    and stop. The document should only be saved inside a claimed WU worktree.
...
If the user provided a document during setup, do **not** save it on main. Create
an "Ingest product document" WU and stop. Save the doc only inside that WU's
worktree after claim.
...
| Save user-provided docs during setup     | Create a Discovery WU, save in worktree later  |
