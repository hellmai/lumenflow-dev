...
<Aside type="note" title="Architecture context">
  LumenFlow is a [runtime kernel](/kernel/runtime) with pluggable domain [packs](/kernel/packs). The
  CLI commands you'll use below (like `wu:create` and `wu:claim`) are tools in the **Software
  Delivery Pack** — they route through the kernel's execution pipeline for [scope
  enforcement](/kernel/scope-intersection), [policy evaluation](/kernel/policy-engine), and
  [evidence recording](/kernel/evidence-store).
</Aside>
...
<Aside type="tip" title="Running setup yourself?">
  Use the [Manual Quickstart (Humans)](/getting-started/manual-quickstart/) if you are not an AI
  agent.
</Aside>
...
6.  **Initialize LumenFlow**
    ...
    This is the canonical bootstrap-all path. It writes `workspace.yaml` and bootstraps Software
    Delivery settings under `workspace.yaml > software_delivery`.
