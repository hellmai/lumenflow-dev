...
<Aside type="note" title="Architecture context">
  LumenFlow is a [runtime kernel](/concepts/kernel) with pluggable domain [packs](/concepts/packs).
  The CLI commands you'll use below (like `wu:create` and `wu:claim`) are tools in the **Software
  Delivery Pack** — they route through the kernel's execution pipeline for [scope
  enforcement](/concepts/scope-intersection), [policy evaluation](/concepts/policy-engine), and
  [evidence recording](/concepts/evidence).
</Aside>
