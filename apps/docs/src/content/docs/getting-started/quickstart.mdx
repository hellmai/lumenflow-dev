...
<Aside type="tip" title="Why Install as a Dev Dependency?">
  Installing `@lumenflow/cli` as a dev dependency ensures your whole team uses the same version,
  and the 40+ CLI commands (like `wu-claim`, `gates`, `flow-report`) are always available via
  `pnpm exec <command>`.
</Aside>
