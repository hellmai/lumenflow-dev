...
Setup is incomplete. Do not install Husky, copy safe-git, or edit hooks during setup.

1. Tell the user: "Setup incomplete—enforcement failed (Husky/safe-git). Creating a WU to fix this."
2. Create a WU: `pnpm wu:create --lane "Operations: Infrastructure" --title "Fix LumenFlow enforcement" --type refactor`
3. Stop. Do not proceed to create feature WUs or claim work.

If the user explicitly accepts incomplete enforcement, record that in the WU notes and stop anyway. No repo writes occur after setup completes.
...
2.  **Initialize the repo** (only if empty)
    ...
    <Aside type="caution" title="Non-Node projects still need a package.json">
      ...
      If there is no `package.json`, ask permission to run `pnpm init`. Do not scaffold a Node stack
      or change the project's framework.
    </Aside>
