...
LumenFlow is an open-source delivery methodology for AI-augmented software teams. It provides a structured workflow where humans and AI agents collaborate using Work Units (WUs), automated gates, and lane-based flow.
...
The CLI and methodology are open source (Apache-2.0). The GitHub App has free and paid tiers based on usage.
...
Only for pre-existing failures with proper documentation:
...
`pnpm wu:done` automatically:

1. Runs gates in the worktree
2. Merges to main (fast-forward)
3. Creates the stamp
...
The CLI provides the full LumenFlow experience (worktrees, gates, memory). Without it, you can still use LumenFlow with manual WU YAML management and the GitHub App.

### What Node.js version do I need?

Node.js 18 or higher. We recommend Node.js 22.
...
No. The CLI works standalone. The GitHub App adds enforcement and team visibility.

### How do I get a LUMENFLOW_TOKEN?

Sign up at [lumenflow.dev](https://lumenflow.dev), create an organization, and generate a token in settings.
...
- [GitHub Discussions](https://github.com/hellmai/os/discussions) - Ask the community
