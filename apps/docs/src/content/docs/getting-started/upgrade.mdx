...
Version 2.1.2 includes these key improvements:
...
- **Guard worktree detection** — `guard-main-branch` now correctly allows operations when running from inside a worktree on a lane branch
...
# 3. Verify
pnpm lumenflow --version  # Should show 2.1.2
```
