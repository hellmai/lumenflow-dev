...
<Tabs syncKey="pkg">
  <TabItem label="pnpm">
    ```bash pnpm up @lumenflow/core @lumenflow/cli @lumenflow/memory \ @lumenflow/agent
    @lumenflow/initiatives @lumenflow/shims \ @lumenflow/metrics --latest ```
  </TabItem>
  <TabItem label="npm">
    ```bash npm update @lumenflow/core @lumenflow/cli @lumenflow/memory \ @lumenflow/agent
    @lumenflow/initiatives @lumenflow/shims \ @lumenflow/metrics ```
  </TabItem>
</Tabs>
...
Version 2.1.0 includes these key improvements:
...
# 3. Verify
pnpm lumenflow --version  # Should show 2.1.0
```
...
- State directory moved from `._legacy/` to `.lumenflow/`
