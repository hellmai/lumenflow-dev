...
pnpm initiative:edit --id INIT-001 --add-phase "Phase 3: Polish"
```
...
# Initialize LumenFlow (use --merge for existing repos with AGENTS.md)
...
<Aside type="tip" title="Use --merge for existing projects">
  The `--merge` flag preserves your existing AGENTS.md content while adding LumenFlow's managed
  section. LumenFlow uses bounded markers to insert its content without overwriting yours.
</Aside>
...
| `pnpm exec lumenflow commands` | List all available commands |
