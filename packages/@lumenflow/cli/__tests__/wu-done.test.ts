  ...
  describe('exposure validation', () => {
    it('should accept --docs-only for WU with exposure: documentation', () => {
      const wu = {
        ...
        type: 'documentation',
        code_paths: ['docs/_frameworks/lumenflow/test.md'],
      };
    ...
    it('should accept --docs-only for WU with docs-only code_paths (auto-detect)', () => {
      ...
      const wu = {
        id: TEST_WU_ID,
        type: 'documentation',
        code_paths: ['docs/_frameworks/lumenflow/playbook.md', 'CLAUDE.md'],
      };
...
describe('WU-1634: mode-execution failure messaging', () => {
  it('surfaces root error context and retry guidance before exiting', async () => {
    const source = await readFile(
      new URL('../src/wu-done-mode-execution.ts', import.meta.url),
      'utf-8',
    );
