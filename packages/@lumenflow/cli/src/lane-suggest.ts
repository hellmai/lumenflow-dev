 ...
 *   pnpm lane:suggest --output workspace.lanes.yaml # Write workspace lane snippet
...
/**
 * Generate a workspace lane-definition snippet from suggestions
 */
function generateWorkspaceLaneDefinitionsYAML(suggestions: LaneSuggestion[]): string {
  const definitions = suggestions.map((suggestion) => ({
    name: suggestion.lane,
    description: suggestion.description,
    wip_limit: 1,
    code_paths: suggestion.code_paths,
  }));

  const header = `# Workspace Lane Definitions
# Generated by: pnpm lane:suggest
# Copy or merge this block into workspace.yaml.
# Source of truth: software_delivery.lanes.definitions
...
function getSuggestions(context: ProjectContext, noLlm: boolean): LaneSuggestion[] {
  if (noLlm) {
    ...
    console.log(chalk.gray('  Using heuristic-based suggestions'));
...
function writeToFile(suggestions: LaneSuggestion[], outputFile: string, projectRoot: string): void {
  ...
  const yamlContent = generateWorkspaceLaneDefinitionsYAML(suggestions);
  ...
  console.log(chalk.green(`\nWorkspace lane snippet written to: ${outputPath}`));
}
...
function showSaveInstructions(): void {
  ...
  console.log(chalk.gray('  pnpm lane:suggest --output workspace.lanes.yaml'));
  console.log(chalk.gray('  pnpm lane:suggest --interactive --output workspace.lanes.yaml'));
  console.log(chalk.gray('  pnpm lane:suggest --json > lanes.json'));
  console.log(chalk.gray('  # Then merge software_delivery.lanes.definitions into workspace.yaml'));
}
...
// Export for testing
export { parseOptions, formatSuggestion, interactiveMode, generateWorkspaceLaneDefinitionsYAML };
