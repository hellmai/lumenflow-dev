...
export async function main() {
  ...
  // WU-1246 + WU-2208 + WU-2209: Auto-generate WU ID if not provided
  // WU-2208: Use remote-aware ID generation to prevent cross-machine collisions
  // WU-2209: Use push collision retry for TOCTOU race between concurrent creates
  let wuId: string;
  const isAutoGeneratedId = !args.id;
  if (args.id) {
    wuId = args.id;
  ...
  try {
    ...
    if (cloudCtx.skipMicroWorktree) {
      ...
      // WU-2209: Standard path with push collision retry for auto-generated IDs.
      // Wraps the micro-worktree transaction so that on push failure due to
      // duplicate ID (TOCTOU race), it re-fetches remote, regenerates the ID,
      // and retries the entire create-commit-push cycle.
      ...
      try {
        const result = await retryCreateOnPushCollision({
          git: getGitForCwd(),
          createFn: async (retryWuId: string) => {
            await withMicroWorktree({
              ...
              execute: async ({ worktreePath }) => {
                ...
                const shortTitle = truncateTitle(args.title);
                const commitMessage = COMMIT_FORMATS.CREATE(retryWuId, shortTitle);
                return {
                  commitMessage,
        ...
        // Update wuId in case it was regenerated during retry
        wuId = result.wuId;
        if (result.attempts > 1) {
          console.log(
            `${LOG_PREFIX} Resolved push collision after ${result.attempts} attempts (final ID: ${wuId})`,
          );
        }
      } finally {
      ...
      // Standard path: micro-worktree isolation (explicit --id provided, no collision retry)
      const previousWuTool = process.env[ENV_VARS.WU_TOOL];
      process.env[ENV_VARS.WU_TOOL] = OPERATION_NAME;
      try {
        await withMicroWorktree({
