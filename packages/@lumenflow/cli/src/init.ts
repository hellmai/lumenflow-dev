...
// Template for .lumenflow.yaml
const LUMENFLOW_YAML_TEMPLATE = `# LumenFlow Configuration
# Generated by: lumenflow init

version: '1.0'

# Project lanes - customize based on your codebase structure
lanes:
  - name: Core
    paths:
      - 'src/core/**'
...
memory:
  checkpoint_interval: 30  # tool calls between auto-checkpoints
`;
...
export async function scaffoldProject(
  ...
  // Create ._legacy/stamps directory
  const legacyStampsDir = path.join(targetDir, '._legacy', 'stamps');
  if (!fs.existsSync(legacyStampsDir)) {
    fs.mkdirSync(legacyStampsDir, { recursive: true });
    result.created.push('._legacy/stamps');
  }
