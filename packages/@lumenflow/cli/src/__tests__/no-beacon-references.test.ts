/**
 * @file no-legacy-references.test.ts
 * Guardrail test: `._legacy` is legacy and must not be referenced in docs/templates/scripts (WU-1447).
 *
...
function repoRootFromThisFile(): string {
  // packages/@lumenflow/cli/src/__tests__/no-legacy-references.test.ts -> repo root (../../../../../)
  const thisDir = path.dirname(fileURLToPath(import.meta.url));
  return path.resolve(thisDir, '..', '..', '..', '..', '..');
}

describe('no legacy ._legacy references (WU-1447)', () => {
  it('should not contain ._legacy references in tracked onboarding docs/templates/scripts', () => {
    ...
    for (const relPath of files) {
      ...
      expect(content, `${relPath} should not reference ._legacy`).not.toContain('._legacy');
    }
  });
});
