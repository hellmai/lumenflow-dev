...
import { parseYAML, stringifyYAML, readWU } from '@lumenflow/core/lib/wu-yaml.mjs';
...
import { ensureOnMain, ensureMainUpToDate, validateWUIDFormat } from '@lumenflow/core/lib/wu-helpers.mjs';
import { withMicroWorktree } from '@lumenflow/core/lib/micro-worktree.mjs';
import { validateLaneFormat } from '@lumenflow/core/lib/lane-checker.mjs';
...
import { validateReadyWU } from '@lumenflow/core/lib/wu-schema.mjs';
import { execSync } from 'node:child_process';
// WU-1442: Import date normalization to fix date corruption from js-yaml
import { normalizeToDateString } from '@lumenflow/core/lib/date-utils.mjs';
// WU-1929: Import initiative-related modules for bidirectional initiative updates
import { INIT_PATTERNS } from '@lumenflow/initiatives/lib/initiative-constants.mjs';
import { INIT_PATHS } from '@lumenflow/initiatives/lib/initiative-paths.mjs';
import { readInitiative, writeInitiative } from '@lumenflow/initiatives/lib/initiative-yaml.mjs';
// WU-2004: Import schema normalization for legacy WU formats
import { normalizeWUSchema } from '@lumenflow/core/lib/wu-schema-normalization.mjs';
// WU-2253: Import WU spec linter for acceptance/code_paths validation
import { lintWUSpec, formatLintErrors } from '@lumenflow/core/lib/wu-lint.mjs';
...
async function main() {
  ...
  if (isDone) {
    ...
    if (opts.acceptance && opts.acceptance.length > 0) disallowedEdits.push('--acceptance');
    if (opts.notes) disallowedEdits.push('--notes');
    if (opts.codePaths && opts.codePaths.length > 0) disallowedEdits.push('--code-paths');
    ...
    if (opts.testPathsE2e && opts.testPathsE2e.length > 0) disallowedEdits.push('--test-paths-e2e');
