 ...
 * 5) Create `._legacy/stamps/WU-{id}.done` in worktree
 ...
 * 13) Emit telemetry to ._legacy/flow.log
 *
...
import { getGitForCwd } from '@lumenflow/core/lib/git-adapter.mjs';
import { die } from '@lumenflow/core/lib/error-handler.mjs';
...
import { validateWUCodePaths } from '@lumenflow/core/lib/code-path-validator.mjs';
// WU-1983: Migration deployment utilities
import {
  discoverLocalMigrations,
  hasMigrationChanges,
  formatMigrationReport,
} from '@lumenflow/core/lib/migration-deployer.mjs';
import { validateDocsOnly, getAllowedPathsDescription } from '@lumenflow/core/lib/docs-path-validator.mjs';
import { scanLogForViolations, rotateLog } from '@lumenflow/core/lib/commands-logger.mjs';
import { rollbackFiles } from '@lumenflow/core/lib/rollback-utils.mjs';
import {
  validateInputs,
  detectModeAndPaths,
  defaultBranchFrom,
...
} from '@lumenflow/core/lib/wu-done-validators.mjs';
// WU-1825: validateCodePathsExist moved to unified code-path-validator
import { validateCodePathsExist } from '@lumenflow/core/lib/code-path-validator.mjs';
import {
  BRANCHES,
  REMOTES,
  PATTERNS,
...
import { printGateFailureBox, printStatusPreview } from '@lumenflow/core/lib/wu-done-ui.mjs';
import { ensureOnMain } from '@lumenflow/core/lib/wu-helpers.mjs';
import { WU_PATHS } from '@lumenflow/core/lib/wu-paths.mjs';
import { writeWU, appendNote, parseYAML } from '@lumenflow/core/lib/wu-yaml.mjs';
import {
  PLACEHOLDER_SENTINEL,
  validateWU,
  validateDoneWU,
  validateApprovalGates,
} from '@lumenflow/core/lib/wu-schema.mjs';
import { validateBacklogSync } from '@lumenflow/core/lib/backlog-sync-validator.mjs';
import { executeBranchOnlyCompletion } from '@lumenflow/core/lib/wu-done-branch-only.mjs';
import { executeWorktreeCompletion, autoRebaseBranch } from '@lumenflow/core/lib/wu-done-worktree.mjs';
import { checkWUConsistency } from '@lumenflow/core/lib/wu-consistency-checker.mjs';
// WU-1542: Use blocking mode compliance check (replaces non-blocking checkMandatoryAgentsCompliance)
import { checkMandatoryAgentsComplianceBlocking } from '@lumenflow/core/lib/orchestration-rules.mjs';
import { endSessionForWU } from '@lumenflow/agent/lib/auto-session-integration.mjs';
import { runBackgroundProcessCheck } from '@lumenflow/core/lib/process-detector.mjs';
import { WUStateStore } from '@lumenflow/core/lib/wu-state-store.mjs';
// WU-1588: INIT-007 memory layer integration
import { createCheckpoint } from '@lumenflow/memory/lib/mem-checkpoint-core.mjs';
import { createSignal, loadSignals } from '@lumenflow/memory/lib/mem-signal-core.mjs';
// WU-1763: Memory store for loading discoveries (lifecycle nudges)
import { loadMemory, queryByWu } from '@lumenflow/memory/lib/memory-store.mjs';
// WU-1943: Checkpoint warning helper
import { hasSessionCheckpoints } from '@lumenflow/core/lib/wu-done-worktree.mjs';
// WU-1603: Atomic lane locking - release lock on WU completion
import { releaseLaneLock } from '@lumenflow/core/lib/lane-lock.mjs';
...
import {
  ...
  canSkipGates,
  clearCheckpoint,
} from '@lumenflow/core/lib/wu-checkpoint.mjs';
// WU-1946: Spawn registry for tracking sub-agent spawns
import { SpawnRegistryStore } from '@lumenflow/core/lib/spawn-registry-store.mjs';
import { SpawnStatus } from '@lumenflow/core/lib/spawn-registry-schema.mjs';
// WU-1999: Exposure validation for UI pairing
// WU-2022: Feature accessibility validation (blocking)
import { validateExposure, validateFeatureAccessibility } from '@lumenflow/core/lib/wu-validation.mjs';
...
const WU_EVENTS_PATH = '._legacy/state/wu-events.jsonl';
...
async function validateClaimMetadataBeforeGates(id, worktreePath, yamlStatus) {
  ...
  const stateDir = path.join(resolvedWorktreePath, '._legacy', 'state');
 ...
 * @param {string} baseDir - Base directory containing ._legacy/state/
 * @returns {Promise<void>}
 */
export async function updateSpawnRegistryOnCompletion(id, baseDir = process.cwd()) {
  try {
    const store = new SpawnRegistryStore(path.join(baseDir, '._legacy', 'state'));
    await store.load();
...
export { shouldSkipWebTests as isDocsOnlyByPaths } from '@lumenflow/core/lib/path-classifiers.mjs';
 ...
 *
 * Scans ._legacy/commands.log for destructive git commands executed during
...
export function emitTelemetry(event) {
  const logPath = path.join('._legacy', 'flow.log');
...
async function auditSkipGates(id, reason, fixWU, worktreePath) {
  const auditPath = path.join('._legacy', 'skip-gates-audit.log');
...
async function auditSkipCosGates(id, reason) {
  const auditPath = path.join('._legacy', 'skip-cos-gates-audit.log');
  const auditDir = path.dirname(auditPath);
  if (!existsSync(auditDir)) mkdirSync(auditDir, { recursive: true });
  const gitAdapter = getGitForCwd();
  const userName = await gitAdapter.getConfigValue(GIT_CONFIG_USER_NAME);
...
async function validateStagedFiles(id, isDocsOnly = false) {
  ...
  const unexpected = staged.filter((file) => {
    ...
    // Whitelist ._legacy/stamps/** pattern
    if (file.startsWith('._legacy/stamps/')) return false;
    return true;
  });
  ...
  if (unexpected.length > 0) {
    ...
    if (otherWuYamlOnly) {
      ...
      die(
        `Unexpected files staged (only current WU YAML, status.md, backlog.md, ._legacy/stamps/<id>.done allowed):\n  ${unexpected.join(`${STRING_LITERALS.NEWLINE}  `)}`
      );
    }
  }
}
...
function auditOwnershipOverride(auditEntry) {
  const auditPath = path.join('._legacy', 'ownership-override-audit.log');
  const auditDir = path.dirname(auditPath);
  if (!existsSync(auditDir)) mkdirSync(auditDir, { recursive: true });
...
async function executeGates({ id, args, isBranchOnly, isDocsOnly, worktreePath, branchName }) {
  ...
  const { runInvariants } = await import('./lib/invariants-runner.mjs');
 ...
 *
 * @param {string} baseDir - Base directory containing ._legacy/memory/
...
async function loadDiscoveriesForWU(baseDir, wuId) {
  try {
    const memory = await loadMemory(path.join(baseDir, '._legacy/memory'));
