...
import { EXIT_CODES, LOG_PREFIX, PATTERNS } from '@lumenflow/core/lib/wu-constants.mjs';
import {
  runConsistencyRepairMode,
  runClaimRepairMode,
  runAdminRepairMode,
} from '@lumenflow/core/lib/wu-repair-core.mjs';
import { repairStateFile, WU_EVENTS_FILE_NAME } from '@lumenflow/core/lib/wu-state-store.mjs';
 ...
 * @param {string} [options.path] - Path to state file (defaults to ._legacy/state/wu-events.jsonl)
 * @returns {Promise<{success: boolean, exitCode: number}>}
 */
async function runStateRepairMode(options) {
  // Default path is ._legacy/state/wu-events.jsonl relative to cwd
  const defaultPath = path.join(process.cwd(), '._legacy', 'state', WU_EVENTS_FILE_NAME);
