...
import { WU_STATUS, PATTERNS, FILE_SYSTEM, EMOJI } from '@lumenflow/core/lib/wu-constants.mjs';
// WU-1603: Check lane lock status before spawning
import { checkLaneLock } from '@lumenflow/core/lib/lane-lock.mjs';
import { minimatch } from 'minimatch';
// WU-2252: Import invariants loader for spawn output injection
import { loadInvariants, INVARIANT_TYPES } from '@lumenflow/core/lib/invariants-runner.mjs';
import {
  validateSpawnArgs,
...
} from '@lumenflow/core/lib/wu-spawn-helpers.mjs';
import { parseAgentSkills } from '@lumenflow/agent/lib/agent-skill-loader.mjs';
import { validateSpawnDependencies, formatDependencyError } from '@lumenflow/core/lib/dependency-validator.mjs';
...
When creating \`._legacy/\` stamps or other artifacts:
...
mkdir -p "$WORKTREE_ROOT/._legacy/agent-runs"
touch "$WORKTREE_ROOT/._legacy/agent-runs/legacy-guardian.stamp"

# WRONG: Hardcoded path to main
# touch /path/to/main/._legacy/agent-runs/legacy-guardian.stamp
\`\`\`
...
- Follow design system tokens in packages/@/design-system
...
- Event tracking in ._legacy/state/wu-events.jsonl
...
`
  ...
  if (laneParent === 'Intelligence') {
    if (!agentSkills.includes('legacy-compliance')) {
      contextHints.push('- `legacy-compliance` — Intelligence lane requires Legacy validation');
    }
...
| legacy-compliance | Code touches LLM, prompts, classification |
 ...
 * @returns {import('./lib/lane-lock.mjs').LockMetadata|null} Lock metadata if occupied, null otherwise
 */
 ...
 *
 * @param {import('./lib/lane-lock.mjs').LockMetadata} lockMetadata - Lock metadata
...
async function main() {
  ...
  if (lane) {
    ...
    if (existingLock && existingLock.wuId !== id) {
      ...
      const { isLockStale } = await import('./lib/lane-lock.mjs');
