 ...
 * Creates ._legacy/memory/ directory with memory.jsonl and config.yaml.
 *
 * Includes audit logging to ._legacy/telemetry/tools.ndjson.
 *
 * Usage:
...
import { initMemory } from '@lumenflow/memory/lib/mem-init-core.mjs';
import { createWUParser } from '@lumenflow/core/lib/arg-parser.mjs';
import { EXIT_CODES, STRING_LITERALS } from '@lumenflow/core/lib/wu-constants.mjs';
 ...
 * Audit log file path
 */
const AUDIT_LOG_PATH = '._legacy/telemetry/tools.ndjson';
...
async function main() {
  ...
  if (!result.alreadyInitialized) {
    ...
    if (result.created.directory) {
      console.log('  - ._legacy/memory/ directory');
    }
