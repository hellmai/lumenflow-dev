...
import { LUMENFLOW_PATHS, DIRECTORIES } from '../wu-constants.mjs';
...
describe('docs-path-validator', () => {
  ...
  it('allows tooling-managed WU lifecycle event log', () => {
    const wuEventsPath = path.posix.join(LUMENFLOW_PATHS.STATE_DIR, WU_EVENTS_FILE_NAME);
  ...
  it('allows stamp files', () => {
    const stampPath = path.posix.join(LUMENFLOW_PATHS.STAMPS_DIR, 'WU-123.done');
  ...
  it('describes the allowed event log path', () => {
    ...
    assert.ok(description.includes(`${LUMENFLOW_PATHS.STATE_DIR}/${WU_EVENTS_FILE_NAME}`));
  });
});
