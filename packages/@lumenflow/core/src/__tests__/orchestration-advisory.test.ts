/**
 * Orchestration Advisory Emitter Tests
 *
 * Tests the mandatory agent advisory emission and compliance checking.
 *
 * @module orchestration-advisory.test
 */
...
describe('orchestration-advisory', () => {
  ...
  describe('emitMandatoryAgentAdvisory', () => {
    ...
    it('emits legacy-guardian advisory for prompts paths', () => {
      emitMandatoryAgentAdvisory(['src/prompts/system.ts'], 'WU-1234');

      const allOutput = consoleLogSpy.mock.calls.map((call) => call[0]).join('\n');
      expect(allOutput).toContain('legacy-guardian');
    });

    it('emits legacy-guardian advisory for classification paths', () => {
      emitMandatoryAgentAdvisory(
        ['packages/application/src/classification/detector.ts'],
        'WU-1234'
      );
  ...
  describe('checkMandatoryAgentsCompliance', () => {
    ...
    it('returns compliant=false with missing legacy-guardian for prompts paths', () => {
      const result = checkMandatoryAgentsCompliance(['src/prompts/system.ts'], 'WU-1234');
      expect(result.compliant).toBe(false);
      expect(result.missing).toContain('legacy-guardian');
    });
