 ...
 * - safety-critical: Tests for red-flag, PII, escalation (always run)
 * - high-risk: Auth, PII, RLS code changes (run integration tests)
 */
...
describe('SAFETY_CRITICAL_TEST_PATTERNS', () => {
  ...
  it('should include PIIpatterns', () => {
    const hasPhi = SAFETY_CRITICAL_TEST_PATTERNS.some(
      (p) => p.toLowerCase().includes('phi')
    );
    assert.ok(hasPhi, 'Should include PIItest patterns');
  });
...
describe('HIGH_RISK_PATH_PATTERNS', () => {
  ...
  it('should include PIIpaths', () => {
    const hasPhi = HIGH_RISK_PATH_PATTERNS.some(
      (p) => p.toLowerCase().includes('phi')
    );
    assert.ok(hasPhi, 'Should include PIIpath patterns');
  });
...
describe('isHighRiskPath', () => {
  ...
  it('should identify PIIpaths', () => {
    expect(isHighRiskPath('src/lib/phi/patterns.ts')).toBe(true);
    expect(isHighRiskPath('packages/@/infrastructure/src/phi/detector.ts')).toBe(true);
  });
...
describe('detectRiskTier', () => {
  ...
  describe('high-risk tier', () => {
    ...
    it('should detect high-risk when PIIfiles are changed', () => {
  ...
  describe('safety-critical detection', () => {
    ...
    it('should include the safety-critical patterns for filtering tests', () => {
      ...
      expect(result.safetyCriticalPatterns.some((p) => p.includes('phi') || p.includes('PII'))).toBeTruthy();
