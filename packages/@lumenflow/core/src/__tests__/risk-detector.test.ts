...
describe('isHighRiskPath', () => {
  ...
  it('should identify PIIpaths', () => {
    ...
    assert.equal(isHighRiskPath('packages/@/infrastructure/src/phi/detector.ts'), true);
  });
...
describe('detectRiskTier', () => {
  ...
  describe('safety-critical detection', () => {
    ...
    it('should include the safety-critical patterns for filtering tests', () => {
      ...
      assert.ok(result.safetyCriticalPatterns.some((p) => p.includes('phi') || p.includes('PII')));
