...
describe('isSkipWebTestsPath', () => {
  describe('documentation paths', () => {
    ...
    it('returns true for ai/ paths', () => {
      expect(
        isSkipWebTestsPath(
          'docs/04-operations/_frameworks/lumenflow/agent/onboarding/workspace-modes.md',
        ),
      ).toBe(true);
    });
  ...
  describe('application paths (should NOT skip tests)', () => {
    ...
    it('returns false for packages/ paths', () => {
      assert.strictEqual(
        isSkipWebTestsPath('packages/@/application/src/index.ts'),
        false,
      );
    });
...
describe('validatePostMutation (WU-1617)', () => {
  ...
  beforeEach(() => {
    ...
    stampPath = join(testDir, '._legacy', 'stamps', 'WU-1617.done');
  });
