...
describe('logs tool', () => {
  beforeEach(() => {
    ...
    mkdirSync(path.join(TEST_FIXTURE_DIR, '._legacy'), { recursive: true });
  });
  ...
  describe('aggregateLogs', () => {
    ...
    beforeEach(async () => {
      ...
      writeFileSync(
        path.join(TEST_FIXTURE_DIR, '._legacy', 'commands.log'),
        COMMANDS_LOG_ENTRY + '\n',
      );
      writeFileSync(path.join(TEST_FIXTURE_DIR, '._legacy', 'flow.log'), FLOW_LOG_ENTRY + '\n');
      writeFileSync(
