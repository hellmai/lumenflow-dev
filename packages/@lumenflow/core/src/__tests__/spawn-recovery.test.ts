...
describe('spawn-recovery', () => {
  ...
  beforeEach(async () => {
    ...
    locksDir = path.join(testDir, '._legacy', 'locks');
    registryDir = path.join(testDir, '._legacy', 'state');
    memoryDir = path.join(testDir, '._legacy', 'state');
    recoveryDir = path.join(testDir, '._legacy', 'recovery');
  });
  ...
  describe('recoverStuckSpawn() - zombie lock detection', () => {
    ...
    it('should create audit log in ._legacy/recovery/', async () => {
      const spawn = FIXTURES.spawnEvent();
      const lock = FIXTURES.lockMetadata({ pid: 99999999 }); // Non-existent PID
