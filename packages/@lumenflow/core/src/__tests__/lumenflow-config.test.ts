...
describe('LumenFlow Config Schema', () => {
  ...
  describe('LegacyPathsSchema', () => {
    it('should provide ._legacy defaults', () => {
      const result = LegacyPathsSchema.parse({});
      expect(result.base).toBe('._legacy');
      expect(result.stampsDir).toBe('._legacy/stamps');
      expect(result.stateDir).toBe('._legacy/state');
    });
  });
  ...
  describe('LumenFlowConfigSchema', () => {
    it('should parse empty object with all defaults', () => {
      ...
      expect(result._legacy).toBeTruthy();
