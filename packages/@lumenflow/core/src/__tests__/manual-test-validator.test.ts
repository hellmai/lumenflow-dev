...
describe('containsHexCoreCode', () => {
  ...
  it('returns true for prompts package files', () => {
    const codePaths = ['packages/@/prompts/src/templates/triage.ts'];

    assert.strictEqual(containsHexCoreCode(codePaths), true);
  });

  it('returns false when only infrastructure files', () => {
    const codePaths = ['packages/@/infrastructure/src/adapters/db.ts'];
...
describe('isCodeFile', () => {
  it('returns true for common code file extensions', () => {
    ...
    assert.strictEqual(isCodeFile('packages/@/application/src/usecase.ts'), true);
  ...
  describe('WUs with hex core code', () => {
    it('passes when unit tests provided', () => {
      const doc = {
        ...
        lane: 'Core Systems',
        type: 'feature',
        code_paths: ['packages/@/application/src/usecases/foo.ts'],
        tests: {
          unit: ['packages/@/application/src/usecases/__tests__/foo.test.ts'],
          manual: [],
          e2e: [],
        },
      };
    ...
    it('passes when integration tests provided', () => {
      const doc = {
        ...
        tests: {
          unit: [],
          manual: [],
          integration: ['packages/@/application/src/__tests__/integration/foo.test.ts'],
        },
      };
