...
describe('containsHexCoreCode', () => {
  ...
  it('returns true for prompts package files', () => {
    const codePaths = ['packages/@patientpath/prompts/src/templates/triage.ts'];

    expect(containsHexCoreCode(codePaths)).toBe(true);
  });

  it('returns false when only infrastructure files', () => {
    const codePaths = ['packages/@patientpath/infrastructure/src/adapters/db.ts'];
...
describe('isCodeFile', () => {
  it('returns true for common code file extensions', () => {
    ...
    expect(isCodeFile('packages/@patientpath/application/src/usecase.ts')).toBe(true);
  ...
  describe('WUs with hex core code', () => {
    it('passes when unit tests provided', () => {
      const doc = {
        ...
        lane: 'Core Systems',
        type: 'feature',
        code_paths: ['packages/@patientpath/application/src/usecases/foo.ts'],
        tests: {
          unit: ['packages/@patientpath/application/src/usecases/__tests__/foo.test.ts'],
          manual: [],
          e2e: [],
        },
      };
    ...
    it('passes when integration tests provided', () => {
      const doc = {
        ...
        tests: {
          unit: [],
          manual: [],
          integration: ['packages/@patientpath/application/src/__tests__/integration/foo.test.ts'],
        },
      };
