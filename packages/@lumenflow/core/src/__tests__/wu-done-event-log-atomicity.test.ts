...
describe('WU-1752: wu:done event log atomicity', () => {
  ...
  beforeEach(async () => {
    ...
    await writeFile(
      path.join('._legacy', 'state', 'wu-events.jsonl'),
      [
        JSON.stringify({
          type: 'claim',
          wuId: 'WU-123',
  ...
  it('collectMetadataToTransaction does not mutate wu-events.jsonl before tx.commit()', async () => {
    ...
    const before = readText(path.join('._legacy', 'state', 'wu-events.jsonl'));
    ...
    await collectMetadataToTransaction({
      ...
      stampPath: path.join('._legacy', 'stamps', 'WU-123.done'),
      transaction,
    });

    const afterCollect = readText(path.join('._legacy', 'state', 'wu-events.jsonl'));
    expect(afterCollect).toBe(before);

    const commitResult = transaction.commit();
    expect(commitResult.success).toBe(true);

    const afterCommit = readText(path.join('._legacy', 'state', 'wu-events.jsonl'));
