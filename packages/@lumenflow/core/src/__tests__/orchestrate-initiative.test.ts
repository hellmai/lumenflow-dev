...
describe('orchestrate-initiative checkpoint-per-wave', () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(join(testDir, '._legacy/stamps'), { recursive: true });
    mkdirSync(join(testDir, '._legacy/artifacts/waves'), { recursive: true });
  });
  ...
  function createStamp(wuId) {
    const stampPath = join(testDir, '._legacy/stamps', `${wuId}.done`);
  ...
  function readWaveManifest(initId, waveNum) {
    const manifestPath = join(testDir, '._legacy/artifacts/waves', `${initId}-wave-${waveNum}.json`);
  ...
  describe('buildCheckpointWave', () => {
    ...
    it('should auto-detect wave number based on existing manifests', async () => {
      ...
      const wave0ManifestPath = join(testDir, '._legacy/artifacts/waves', 'INIT-001-wave-0.json');
      writeFileSync(
        wave0ManifestPath,
  ...
  describe('formatCheckpointOutput', () => {
    ...
    it('should produce structured output with wave info and resume instructions', async () => {
      ...
      const output = formatCheckpointOutput({
        ...
        manifestPath: '._legacy/artifacts/waves/INIT-001-wave-0.json',
      });
