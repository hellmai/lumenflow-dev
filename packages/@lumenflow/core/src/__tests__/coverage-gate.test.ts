...
describe('isHexCoreFile', () => {
  it('returns true for application package files', () => {
    assert.strictEqual(
      isHexCoreFile('packages/@patientpath/application/src/usecases/foo.ts'),
      true
    );
    assert.strictEqual(
      isHexCoreFile('packages/@patientpath/application/src/domain/entity.ts'),
      true
    );
  });
  ...
  it('returns false for infrastructure package files', () => {
    assert.strictEqual(
      isHexCoreFile('packages/@patientpath/infrastructure/src/adapters/db.ts'),
      false
    );
  });
...
describe('parseCoverageJson', () => {
  ...
  it('parses coverage-summary.json correctly', () => {
    ...
    assert.ok(result.files['packages/@patientpath/application/src/usecases/foo.ts']);
  });
