...
describe('isHexCoreFile', () => {
  it('returns true for application package files', () => {
    expect(isHexCoreFile('packages/@/application/src/usecases/foo.ts')).toBe(true);
    expect(isHexCoreFile('packages/@/application/src/domain/entity.ts')).toBe(true);
  });
  ...
  it('returns true for absolute application package paths', () => {
    expect(
      isHexCoreFile(join(process.cwd(), 'packages/@/application/src/usecases/foo.ts'))
    ).toBe(true);
  });

  it('returns false for infrastructure package files', () => {
    expect(isHexCoreFile('packages/@/infrastructure/src/adapters/db.ts')).toBe(false);
  });
...
describe('parseCoverageJson', () => {
  ...
  it('parses coverage-summary.json correctly', () => {
    const coverageData = {
      ...
      'packages/@/application/src/usecases/foo.ts': {
    ...
    expect(result.files['packages/@/application/src/usecases/foo.ts']).toBeTruthy();
  });
  ...
  it('blocks when coverage is below threshold in block mode', async () => {
    ...
    try {
      ...
      const coverageData = {
        ...
        [join(process.cwd(), 'packages/@/application/src/usecases/foo.ts')]: {
