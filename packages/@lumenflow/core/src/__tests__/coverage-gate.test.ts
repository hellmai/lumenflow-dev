...
describe('isHexCoreFile', () => {
  it('returns true for application package files', () => {
    expect(isHexCoreFile('packages/@patientpath/application/src/usecases/foo.ts')).toBe(true);
    expect(isHexCoreFile('packages/@patientpath/application/src/domain/entity.ts')).toBe(true);
  });
  ...
  it('returns true for absolute application package paths', () => {
    expect(
      isHexCoreFile(join(process.cwd(), 'packages/@patientpath/application/src/usecases/foo.ts'))
    ).toBe(true);
  });

  it('returns false for infrastructure package files', () => {
    expect(isHexCoreFile('packages/@patientpath/infrastructure/src/adapters/db.ts')).toBe(false);
  });
...
describe('parseCoverageJson', () => {
  ...
  it('parses coverage-summary.json correctly', () => {
    ...
    expect(result.files['packages/@patientpath/application/src/usecases/foo.ts']).toBeTruthy();
  });
  ...
  it('blocks when coverage is below threshold in block mode', async () => {
    ...
    try {
      ...
      const coverageData = {
        ...
        [join(process.cwd(), 'packages/@patientpath/application/src/usecases/foo.ts')]: {
