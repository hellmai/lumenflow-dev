...
describe('wu-checkpoint', () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(path.join(testDir, '._legacy', 'checkpoints'), { recursive: true });
  ...
  describe('createPreGatesCheckpoint()', () => {
    ...
    it('should persist checkpoint to ._legacy/checkpoints/', async () => {
      ...
      const checkpointPath = path.join(testDir, '._legacy', 'checkpoints', 'WU-100.checkpoint.json');
    ...
    it('should include schema version', async () => {
      ...
      const checkpointPath = path.join(testDir, '._legacy', 'checkpoints', 'WU-200.checkpoint.json');
