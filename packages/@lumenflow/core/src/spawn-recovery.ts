 ...
 *
 * All recovery actions are logged to ._legacy/recovery/ for audit.
 *
 * Library-First Note: This is project-specific spawn recovery code for
 * 's custom spawn-registry.jsonl, lane-lock, and memory-store.
 ...
 * @returns {string} Path to ._legacy/recovery/
 */
function getRecoveryDir(baseDir) {
  return path.join(baseDir, '._legacy', RECOVERY_DIR_NAME);
}
...
async function getLastCheckpoint(baseDir, wuId) {
  const memoryDir = path.join(baseDir, '._legacy', 'state');
...
export async function recoverStuckSpawn(spawnId, options = {}) {
  ...
  const registryDir = path.join(baseDir, '._legacy', 'state');
