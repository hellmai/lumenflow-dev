 ...
 * Stores events in ._legacy/state/wu-events.jsonl (append-only, git-friendly).
 *
 * Features:
...
export class WUStateStore {
  /**
   * @param {string} baseDir - Directory containing ._legacy/state/
   */
  constructor(baseDir) {
 ...
 *
 * @example
 * const stateFilePath = path.join(process.cwd(), '._legacy', 'state', 'wu-events.jsonl');
