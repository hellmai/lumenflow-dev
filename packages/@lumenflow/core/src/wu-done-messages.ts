...
export const RECOVERY = {
  ...
  COMMIT_FAILED: (error: unknown): string =>
    `${LOG_PREFIX.DONE} ${EMOJI.WARNING} Could not commit recovery changes: ${formatUnknownError(error)} (metadata still on disk)`,
...
export const WORKTREE = {
  REMOVED: (path: string): string => `${LOG_PREFIX.DONE} ${EMOJI.SUCCESS} Removed worktree ${path}`,
  NOT_FOUND: `${LOG_PREFIX.DONE} Worktree not found; skipping removal`,
  REMOVE_FAILED: (path: string, error: unknown): string =>
    `${LOG_PREFIX.DONE} Could not remove worktree ${path}: ${formatUnknownError(error)}`,
 ...
 */
export const BRANCH = {
  DELETED_LOCAL: (branch: string): string =>
    `${LOG_PREFIX.DONE} ${EMOJI.SUCCESS} Deleted local branch ${branch}`,
  DELETED_REMOTE: (branch: string): string =>
    `${LOG_PREFIX.DONE} ${EMOJI.SUCCESS} Deleted remote branch ${branch}`,
  DELETE_FAILED: (branch: string, error: unknown): string =>
    `${LOG_PREFIX.DONE} Could not delete branch ${branch}: ${formatUnknownError(error)}`,
  DELETE_REMOTE_FAILED: (error: unknown): string =>
    `${LOG_PREFIX.DONE} Could not delete remote branch: ${formatUnknownError(error)}`,
...
export const COMPLETION = {
  SUCCESS_HEADER: (id: string, title: string): string =>
...
export const STATE_TRANSITION = {
  VALIDATION_FAILED: (error: unknown): string =>
    `State transition validation failed: ${formatUnknownError(error)}`,
};
 ...
 */
export const REBASE = {
  STARTING: (branch: string, mainBranch: string): string =>
  ...
  FAILED: (error: unknown): string =>
    `${LOG_PREFIX.DONE} ${EMOJI.FAILURE} Auto-rebase failed: ${formatUnknownError(error)}`,
  ABORTED: `${LOG_PREFIX.DONE} ${EMOJI.INFO} Rebase aborted - worktree left clean`,
  MANUAL_FIX: (worktreePath: string, remote: string, mainBranch: string, branch: string): string =>
...
export const PREFLIGHT = {
  ...
  BRANCH_BEHIND: (commitsBehind: number, threshold: number): string =>
  ...
  BRANCH_INFO: (branch: string, branchTip: string, mergeBase: string, mainHead: string): string =>
  ...
  CONFLICT_ERROR: (remote: string = REMOTES.ORIGIN, mainBranch: string = BRANCHES.MAIN): string =>
  ...
  MERGE_COMMITS_DETECTED: (count: number): string =>
  ...
  EMPTY_MERGE_WARNING: (commitCount: number): string =>
...
export const MERGE = {
  ...
  UPDATED_MAIN: (remote: string): string =>
    `${LOG_PREFIX.DONE} ${EMOJI.SUCCESS} Updated main from ${remote}, retrying merge...`,
  PUSHED: (remote: string, mainBranch: string): string =>
    `${LOG_PREFIX.DONE} ${EMOJI.SUCCESS} Pushed to ${remote}/${mainBranch}`,
  FF_MERGE_ERROR: (
    branch: string,
    mergeError: unknown,
    pullError: unknown,
    remote: string = REMOTES.ORIGIN,
    mainBranch: string = BRANCHES.MAIN,
  ): string =>
    `Fast-forward merge failed and retry with git pull also failed.\n\n` +
    `Original merge error: ${formatUnknownError(mergeError)}\n` +
  ...
  FF_FAILED_NON_DIVERGED_ERROR: (
    ...
    `Underlying git error: ${formatUnknownError(originalError)}\n\n` +
    `Next steps:\n` +
