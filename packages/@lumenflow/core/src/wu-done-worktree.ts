...
export async function executeWorktreeCompletion(context) {
  ...
  if (detectZombieState(docForUpdate, worktreePath)) {
    ...
    if (shouldEscalateToManualIntervention(attemptCount)) {
      ...
      console.log(`${BOX.SIDE}  rm ._legacy/recovery/${id}.recovery`);
      console.log(BOX.BOT);
  ...
  try {
    ...
    const workingEventsPath = path.join('._legacy', 'state', WU_EVENTS_FILE_NAME);
...
const APPEND_ONLY_FILES = [
  ...
  path.join('._legacy', 'state', WU_EVENTS_FILE_NAME),
  // Status and backlog are generated from state store but may conflict during rebase
  WU_PATHS.STATUS(),
  WU_PATHS.BACKLOG(),
];

const WU_EVENTS_PATH = path.join('._legacy', 'state', WU_EVENTS_FILE_NAME);
