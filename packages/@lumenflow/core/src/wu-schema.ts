...
const sharedFields = {
   ...
   * - sensitive_data: Changes to PII data handling
  ...
  escalation_triggers: z
    .array(z.enum(['sensitive_data', 'security_p0', 'budget', 'external_compliance', 'cross_lane_arch']))
    .optional()
    .default([]),
...
export const ESCALATION_TRIGGER_TYPES = [
  'sensitive_data', // PII data handling changes
...
export function detectEscalationTriggers(wu) {
  ...
  // PII: Changes to user data, auth, or medical records
  const phiPatterns = [
    'phi',
    'pii',
    'user',
