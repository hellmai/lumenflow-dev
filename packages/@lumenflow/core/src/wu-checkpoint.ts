...
/**
 * Checkpoint directory within ._legacy
 */
...
function getCheckpointPath(wuId, options = {}) {
  ...
  return path.join(baseDir, '._legacy', CHECKPOINT_DIR, `${wuId}.checkpoint.json`);
}
...
function ensureCheckpointDir(options = {}) {
  const baseDir = options.baseDir || process.cwd();
  const checkpointDir = path.join(baseDir, '._legacy', CHECKPOINT_DIR);
