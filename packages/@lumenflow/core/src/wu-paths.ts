...
const PATH_DEPTHS = {
  ...
  WU_YAML: 5,
  /** State store is 3 levels deep: ._legacy/state/wu-events.jsonl */
  STATE_STORE: 3,
};
 ...
 * then appends ._legacy/state for the state store location.
 *
...
export function getStateStoreDirFromBacklog(backlogPath) {
  ...
  return path.join(repoRoot, '._legacy', 'state');
}
 ...
 * const stampPath = WU_PATHS.STAMP('WU-123'); // '._legacy/stamps/WU-123.done'
 */
export const WU_PATHS = {
   ...
   */
  STAMPS_DIR: () => path.join('._legacy', 'stamps'),
   ...
   */
  STAMP: (id) => path.join('._legacy', 'stamps', `${id}.done`),
};
