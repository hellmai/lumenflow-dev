...
/**
 * WU-1335: Recovery marker subdirectory within ._legacy
 */
 ...
 * @param {string} [baseDir=process.cwd()] - Base directory for ._legacy
 * @returns {string} Path to recovery marker file
 */
export function getRecoveryMarkerPath(id, baseDir = process.cwd()) {
  return join(baseDir, '._legacy', RECOVERY_MARKER_DIR, `${id}.recovery`);
}
...
export function incrementRecoveryAttempt(id, baseDir = process.cwd()) {
  const markerPath = getRecoveryMarkerPath(id, baseDir);
  const markerDir = join(baseDir, '._legacy', RECOVERY_MARKER_DIR);
