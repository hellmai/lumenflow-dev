...
4. VERIFY COMPLETION before reporting success
   - Run: node tools/lib/agent-verification.ts ${id} (from shared checkout)
...
3. **Verify before success**: run \`pnpm gates\` in the worktree, then run \`node tools/lib/agent-verification.ts ${id}\` (from the shared checkout)
...
touch "$WORKTREE_ROOT/.lumenflow/agent-runs/legacy-guardian.stamp"

# WRONG: Hardcoded path to main
# touch /path/to/main/.lumenflow/agent-runs/legacy-guardian.stamp
\`\`\`
...
- From shared checkout: \`node tools/lib/agent-verification.ts ${id}\`
