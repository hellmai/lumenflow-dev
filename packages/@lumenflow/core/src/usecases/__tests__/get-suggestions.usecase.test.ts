...
describe('GetSuggestionsUseCase', () => {
  ...
  const mockWUProgress: WUProgress[] = [
    {
      ...
      dodProgress: 9,
      dodTotal: DOD_TOTAL,
      agents: { 'legacy-guardian': 'pass', 'code-reviewer': 'pending' },
  ...
  describe('with code path analysis', () => {
    it('should add mandatory agent suggestions for matching paths', async () => {
      ...
      const legacySuggestion = result.find((s) => s.action.includes('legacy-guardian'));
      expect(legacySuggestion).toBeDefined();
      expect(legacySuggestion?.priority).toBe('high');
    });

    it('should not duplicate mandatory agent suggestions', async () => {
      // WU already has legacy-guardian pending, code paths also trigger it
      mockCollector.getWUProgress = vi.fn().mockResolvedValue([
        {
          wuId: 'WU-1237',
          lane: 'Intelligence',
          ...
          dodProgress: 5,
          dodTotal: DOD_TOTAL,
          agents: { 'legacy-guardian': 'pending' },
          headline: 'In progress',
        },
      ]);
      ...
      const legacySuggestions = result.filter((s) => s.action.includes('legacy-guardian'));
      // Should only have one suggestion, not duplicated
      expect(legacySuggestions.length).toBe(1);
    });
  });
});
