 ...
 * - Audit logging for telemetry (._legacy/telemetry/tools.ndjson)
 * - Consistent error handling with agent-friendly messages
 *
 * Usage:
 *   import { runTool, ToolRunner, createToolConfig } from './tool-runner.mjs';
 *
 *   // Direct invocation
...
function createAuditLogger() {
  return (entry) => {
    ...
    // Will integrate with ._legacy/telemetry/tools.ndjson in future WU
    void entry;
  };
}
    ...
    if (!tool) {
      return createErrorOutput({
        ...
        tryNext: [
          ...
          'Use runner.listTools() to see available tools',
        ],
      });
    }
