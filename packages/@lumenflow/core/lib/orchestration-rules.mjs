...
import { MANDATORY_TRIGGERS } from './domain/orchestration.constants.mjs';
...
function generateWUSuggestions(wu, nextId) {
  ...
  const mandatoryAgents = ['security-auditor', 'legacy-guardian'];
...
export function buildMandatoryAgentsErrorMessage(wuId, missingAgents, codePaths) {
  ...
  lines.push('  2. For security-auditor: Review PII/auth/RLS implications');
  lines.push('  3. For legacy-guardian: Validate LLM/prompt compliance');
  lines.push('');
  lines.push('To bypass (NOT RECOMMENDED for PII/auth work):');
