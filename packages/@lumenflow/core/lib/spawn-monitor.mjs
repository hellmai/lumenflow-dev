...
import { isZombieLock, readLockMetadata } from './lane-lock.mjs';
import { recoverStuckSpawn, RecoveryAction } from './spawn-recovery.mjs';
import {
  escalateStuckSpawn,
 ...
 * @property {import('./spawn-registry-schema.mjs').SpawnEvent} spawn - The stuck spawn event
 ...
 *
 * @param {import('./spawn-registry-schema.mjs').SpawnEvent[]} spawns - Array of spawn events
...
export async function checkZombieLocks(options = {}) {
  ...
  const locksDir = path.join(baseDir, '._legacy', 'locks');
  const zombies = [];
 ...
 * @param {StuckSpawnInfo[]} stuckSpawns - Array of stuck spawn info
 * @param {Object} [options] - Options
 * @param {string} [options.baseDir] - Base directory for ._legacy/
