 ...
 *
 * Logs to ._legacy/telemetry/prompt-nightly.ndjson and Axiom.
 * Alerts if:
...
import { analyzePrompt } from './token-counter.mjs';
import { readFile, writeFile, mkdir, appendFile, access } from 'node:fs/promises';
import { resolve, dirname } from 'path';
import { fileURLToPath } from 'url';
import { glob } from 'glob';
import { EXIT_CODES, FILE_SYSTEM, STRING_LITERALS } from './wu-constants.mjs';
...
// Paths
const YESTERDAY_METRICS_PATH = resolve(ROOT_DIR, '._legacy/telemetry/prompt-metrics-yesterday.json');
const TODAY_METRICS_PATH = resolve(ROOT_DIR, '._legacy/telemetry/prompt-metrics.json');
const NDJSON_LOG_PATH = resolve(ROOT_DIR, '._legacy/telemetry/prompt-nightly.ndjson');
...
async function monitor() {
  ...
  const pattern = 'packages/@/prompts/**/*.yaml';
