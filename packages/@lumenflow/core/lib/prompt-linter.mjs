...
import { analyzePrompt, getLongestLines } from './token-counter.mjs';
...
const METRICS_CACHE_PATH = resolve(ROOT_DIR, '._legacy/telemetry/prompt-metrics.json');
...
async function log(level, event, data, output = {}) {
  ...
  // For CLI, write to ._legacy/telemetry/prompt-lint.ndjson
  const ndjsonPath = resolve(ROOT_DIR, '._legacy/telemetry/prompt-lint.ndjson');
...
export async function lintPrompts(
  ...
  if (filePaths.length === 0) {
    const pattern = 'packages/@/prompts/orchestrator-*/**/*.yaml';
