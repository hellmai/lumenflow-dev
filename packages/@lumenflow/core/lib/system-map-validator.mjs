 ...
 * 5. No PIIentries tagged for investor/public
 *
...
/**
 * Audiences that should NOT have access to restricted (PII) data
 * These are considered "external" audiences who should never see PII
 * Note: investor CAN see confidential (investor materials ARE confidential)
 * @type {string[]}
 */
const SENSITIVE_RESTRICTED_AUDIENCES = ['investor', 'user', 'operator', 'public'];
...
/**
 * Validate classification prevents PIIrouting to investor/public
 *
 * Rule: restricted (PII) data should NOT be accessible to external audiences
 * - restricted = PII data, must NOT go to investor/patient/operator
...
export function validateClassificationRouting(systemMap) {
  ...
  for (const entry of entries) {
    ...
    // Only check restricted classification (PII data)
    // Confidential is OK for investors (investor materials are confidential by design)
    if (classification !== 'restricted') {
      continue;
    }

    // Check if any PII-restricted audiences have access
    for (const audience of audiences) {
      if (SENSITIVE_RESTRICTED_AUDIENCES.includes(audience)) {
        errors.push(
          `PIIrouting violation: ${entry.id} has restricted (PII) classification but is accessible to '${audience}'`
        );
      }
    }
  }
