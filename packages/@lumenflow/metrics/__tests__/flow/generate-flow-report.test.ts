  ...
  describe('LLM metrics', () => {
    ...
    it('aggregates by type', () => {
      const llmEvents: LLMTelemetryEvent[] = [
        ...
        { timestamp: '2026-01-02T11:00:00Z', eventType: 'llm.classification.complete', classificationType: 'sensitive_detection', durationMs: 200, tokensUsed: 100, estimatedCostUsd: 0.002, confidence: 0.8, fallbackUsed: true },
      ];

      const result = generateFlowReport({ ...baseInput, llmEvents });
      expect(result.llm.byType['mode_detection']).toBeDefined();
      expect(result.llm.byType['sensitive_detection']).toBeDefined();
      expect(result.llm.byType['mode_detection']!.count).toBe(1);
      expect(result.llm.byType['sensitive_detection']!.fallbackRate).toBe('100.0');
    });
