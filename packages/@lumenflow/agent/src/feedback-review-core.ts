 ...
 *
 * Aggregates ._legacy/incidents/*.ndjson and ._legacy/memory/memory.jsonl,
...
/**
 * Load all incidents from ._legacy/incidents/*.ndjson
 *
 * @param {string} baseDir - Base directory
 * @returns {Promise<object[]>} All incident objects
 */
async function loadIncidents(baseDir) {
  const incidentsDir = path.join(baseDir, '._legacy', 'incidents');
  let files;
...
/**
 * Load memory nodes from ._legacy/memory/memory.jsonl
 *
 * @param {string} baseDir - Base directory
 * @returns {Promise<object[]>} Memory node objects
 */
async function loadMemoryNodes(baseDir) {
  const memoryFile = path.join(baseDir, '._legacy', 'memory', 'memory.jsonl');
 ...
 *
 * @param {string} baseDir - Base directory containing ._legacy
