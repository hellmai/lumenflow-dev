 ...
 * Creates ._legacy/memory/ directory with empty memory.jsonl and config.yaml.
 *
 ...
 * @returns {Promise<object>} Result object with success, paths, and created flags
 *
 * @example
...
export async function initMemory(baseDir) {
  ...
  if (!dirExists) {
    // eslint-disable-next-line security/detect-non-literal-fs-filename -- CLI tool creates ._legacy/memory/ directory
  ...
  if (!memoryExists) {
    ...
    await fs.writeFile(memoryJsonlPath, '', 'utf-8');
