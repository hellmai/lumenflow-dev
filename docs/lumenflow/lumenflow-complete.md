...
6. **Safety & governance** — Respect PII/privacy rules, approved sources, RLS policies; when uncertain, choose safer path
...
AI agents using Write/Edit/Read tools can inadvertently bypass worktree isolation by passing absolute paths. Even when the shell working directory is correct (`cd worktrees/lane-wu-xxx`), file operation tools that accept absolute paths will target the main checkout if given `/home/.../.co.uk/apps/...` instead of relative `apps/...`.
...
touch ._legacy/stamps/WU-427.done  # ❌ Manual stamp bypasses validation
```
...
3. **Stamp creation** (`._legacy/stamps/WU-XXX.done`)
...
- [ ] NEVER manually create `._legacy/stamps/*.done` files
...
The shared `main` checkout is for documentation changes only (`docs/**`, `docs/**`, `*.md`, `._legacy/stamps/**`). Hooks block code changes on `main` to prevent accidental manual merges. Claim a WU and work in a dedicated worktree/branch for any code.
...
- **Security/compliance** requirements (RLS policies, PIIlogging rules, auth boundaries)
...
- **Application Logic**: Implement use case in `@/application` (Step 3)
...
8. ✅ `.done` stamp in `._legacy/stamps/WU-XXX.done`
...
5. Update baseline metrics in `._legacy/artifacts/prompts/baselines/<version>.json`
...
These rules prevent incomplete implementations from being marked done. See [llm-migration-audit-2025-10-15.md](../product/legacy/archive/llm-migration-audit-2025-10-15.md) for context.
...
All skip-gates events are logged to `._legacy/skip-gates-audit.log` with timestamp, user, reason, and fix-WU for accountability.
...
- Modifies only: `docs/**`, `docs/**`, `*.md` files, `._legacy/stamps/**`
...
3. ✅ `.done` stamp in `._legacy/stamps/WU-XXX.done`
...
git add memory-bank/ ops/ci/ ._legacy/stamps/ [code files]
...
- **RLS (Row-Level Security)** enforced on all user data
...
### 10.5 Supply-Chain Security (WU-403)
 ...
 requires rigorous supply-chain security for regulated compliance (compliance, ISO 27001, SOC 2).
...
1. **Impact analysis**: Detailed assessment of impact on 
...
Supply-chain security gates run BEFORE application tests in `.github/workflows/legacy-gates.yml`:
...
- Audit trail for regulatory standard assessment
...
- regulatory standard: [Digital Technology Assessment Criteria](https://transform.example.gov/key-tools-and-info/digital-technology-assessment-criteria-compliance/)
...
5. **Stamp and push:** create `._legacy/stamps/WU-####.done`, commit atomically to `main`
...
- **Product**: `{PROJECT_ROOT}/product/legacy-complete.md`
...
- NDJSON logs (`._legacy/telemetry/gates.ndjson`)
