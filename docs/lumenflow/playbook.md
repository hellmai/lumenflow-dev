...
- **Privacy**: PII, PII, GDPR compliance
...
- After tests and gates are green:
  ...
  - This helper performs a complete WU completion sequence:
    ...
    4. Creates `._legacy/stamps/WU-341.done`
...
- No `._legacy/stamps/WU-<id>.done` file created
...
3. **Document the incident:**
   ...
   ```bash
   # Log to audit file
   echo "$(date): Destructive command run on main. Recovery: [describe]" >> ._legacy/incidents.log
...
- **LLM classifier validation**: Mode detection, red flag detection, PIIdetection, entity recognition
...
- **Mode Detection**: `tests/golden/mode-detection.yaml` (60 examples)
  ...
  - UK-specific: regulatory authority terminology, domain terminology
...
- **PIIDetection**: `tests/golden/phi-detection.yaml` (60 examples)
  - Tests: phone, national_id, postcode, email, name, address
  - Edge cases: hospital numbers (not personal), partial info, famous addresses
  - UK-specific: national ID numbers, UK postcodes, UK phone formats
...
| ---------------------------------------------- | ---------- | -------- | --------- | ----- | ---------------------------------------------------- |
| Application (`@/application`)       | ≥90%       | ≥90%     | ≥90%      | ≥90%  | Business logic MUST be comprehensively tested (§2.3) |
| Infrastructure (`@/infrastructure`) | ≥80%       | ≥80%     | ≥80%      | ≥80%  | Adapters require thorough integration tests (§6.4.4) |
| Global (all code)                              | ≥80%       | ≥80%     | ≥80%      | ≥80%  | Maintains minimum quality bar across codebase        |

**CI Enforcement** (WU-1002):

Coverage is automatically measured and enforced in CI via `.github/workflows/legacy-gates.yml`:
...
4. **Audit Trail**: Coverage artifacts downloadable from Actions tab for compliance evidence (regulatory standard)
...
- `--delete-branch` — Delete lane branch after merge (both local and remote)
- `--skip-gates` — **DANGER:** Skip gates check (requires `--reason` + `--fix-wu`, audited to `._legacy/skip-gates-audit.log`)
...
- `--override-owner` — Override ownership check (requires `--reason`, audited to `._legacy/ownership-override-audit.log`)
- `--help, -h` — Show help
...
1. Stage stamp in THIS commit:
   ```bash
   printf "WU WU-<ID> — <title>\nCompleted: <YYYY-MM-DD>\n" > ._legacy/stamps/WU-<ID>.done
   git add ._legacy/stamps/WU-<ID>.done
...
### Available Project Skills
 ...
 provides these skills in `.claude/skills/`:
...
**Activates when**: Code touches LLM features, classification, PII, medical claims, accessibility, or when reviewing WUs.
...
**References**: legacy-complete.md §2.6, §9, §10, §11, §23
...
- `docs/01-product/legacy/legacy-complete.md` - Complete Legacy specification
...
| Review for Legacy compliance | Reads legacy-complete.md (~2500 lines)    | Loads legacy-compliance skill (~300 lines) + checklist          |
...
- `regulated-compliance` - WCAG/PHI/compliance checklists
...
3. Reference canonical docs (lumenflow-complete.md, legacy-complete.md) with section numbers
