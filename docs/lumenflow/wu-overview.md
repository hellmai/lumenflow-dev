...
- We turn the big Legacy spec into many **tiny, testable jobs** called **Work Units (WUs)**.
...
**Outcome:** we move **faster** while staying **safer** (vital for regulated).
...
- Each WU links back to the spec (`{PROJECT_ROOT}/product/legacy-complete.md`) so anyone can trace “why.”
...
- Tests and tiny **artifacts** (e.g., `._legacy/artifacts/spec-linter/summary.json`, `glass-count/web.json`) show the rule is enforced.
...
## Why it’s right for regulated

**Trust & safety built‑in**

- WUs encode rules like **Source Gate** (only show “Cited” when sources are _allow‑listed and fresh_), **PIIguard** (ask before saving sensitive info), and **red‑flag** signposting (111/999).

**Audit‑friendly**

- Artifacts + living docs form a simple audit trail. Good for regulatory standard / internal reviews.
...
- **Proof** → artifacts listed in each WU (`._legacy/artifacts/**`)
...
- **E2E tests** (Claim badge states, PIIsave‑preview, Dock/Space behaviour)
