...
### Taxonomy
 ...
 uses an 8-tier taxonomy with 33 sub-lanes across all parent lanes:

**Intelligence (4 sub-lanes)**

- **Prompts:** Prompt engineering, base prompts, overlays, instruction stacks
- **Classifiers:** LLM classification (mode, red-flag, PII, entity recognition)
...
- **Security:** Authentication, authorization, PIIprotection, secrets management
...
- **Chat:** Legacy chat experience (chat UI, composer, attachments, content blocks)
...
- **Classifiers** — `packages/classifiers/**`, keywords: classifier, mode detection, PII, red flag
...
- **Security** — `docs/04-operations/security/**`, keywords: auth, PII, secrets, encryption, HIPAA
...
- **Chat** — `apps/web/src/components/ui/LegacyChat*.tsx`, `apps/web/src/lib/assistant-ui/**`, keywords: chat, composer, assistant, stream
