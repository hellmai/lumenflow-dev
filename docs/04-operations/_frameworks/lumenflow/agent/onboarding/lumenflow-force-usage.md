...
Analysis of `._legacy/force-bypasses.log` reveals that LUMENFLOW_FORCE is used extensively, but almost exclusively by human operators (user "Tom") rather than AI agents. The vast majority of uses are for legitimate infrastructure operations such as:
...
1. **Audit logging:** All bypasses logged to `._legacy/force-bypasses.log` with timestamp, hook, user, branch, reason, and working directory.

2. **Warning when no reason:** If LUMENFLOW_FORCE_REASON is not provided, a warning is printed.

3. **Git-tracked audit log:** The `._legacy/force-bypasses.log` file is version controlled, providing historical accountability.
...
Establish a monthly review of `._legacy/force-bypasses.log` to identify:
