...
- `._legacy/safety-blocks.log` (blocked safety actions)
- `._legacy/force-bypasses.log` (LUMENFLOW_FORCE bypasses)
...
This will be logged to ._legacy/force-bypasses.log.
...
All bypasses are logged to `._legacy/force-bypasses.log`:
...
# Should succeed and log to ._legacy/force-bypasses.log
```
